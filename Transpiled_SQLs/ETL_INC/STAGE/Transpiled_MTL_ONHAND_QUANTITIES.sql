TRUNCATE table
	table bronze_bec_ods_stg.MTL_ONHAND_QUANTITIES;
INSERT INTO bronze_bec_ods_stg.MTL_ONHAND_QUANTITIES (
  inventory_item_id,
  organization_id,
  date_received,
  last_update_date,
...
(
  ...
  WHERE
    ...
    AND (COALESCE(ONHAND_QUANTITIES_ID, 0), KCA_SEQ_ID) IN (
      ...
      FROM bec_raw_dl_ext.MTL_ONHAND_QUANTITIES_DETAIL
      WHERE
        kca_operation <> 'DELETE' AND is_consigned = 2
      GROUP BY
        (
          COALESCE(ONHAND_QUANTITIES_ID, 0)
        )
    ...
    AND (
      ...
      OR CAST(SUBSTRING(kca_seq_id, 1, 8) AS TIMESTAMP) > (
        SELECT
          (
            executebegints - prune_days
          )
