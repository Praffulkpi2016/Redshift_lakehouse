/* Delete Records */
DELETE FROM silver_bec_ods.msc_system_items
WHERE
  (SR_INSTANCE_ID, PLAN_ID, INVENTORY_ITEM_ID, ORGANIZATION_ID) IN (
    SELECT
      stg.SR_INSTANCE_ID,
      stg.PLAN_ID,
      stg.INVENTORY_ITEM_ID,
      stg.ORGANIZATION_ID
    FROM silver_bec_ods.msc_system_items AS ods, bronze_bec_ods_stg.msc_system_items AS stg
    WHERE
...
/* Soft delete */
UPDATE silver_bec_ods.msc_system_items SET IS_DELETED_FLG = 'N';
UPDATE silver_bec_ods.msc_system_items SET IS_DELETED_FLG = 'Y'
WHERE
  (SR_INSTANCE_ID, PLAN_ID, INVENTORY_ITEM_ID, ORGANIZATION_ID) IN (
    SELECT
      SR_INSTANCE_ID,
    ...
    WHERE
      (SR_INSTANCE_ID, PLAN_ID, INVENTORY_ITEM_ID, ORGANIZATION_ID, KCA_SEQ_ID) IN (
        SELECT
          SR_INSTANCE_ID,
          PLAN_ID,
          INVENTORY_ITEM_ID,
