/* Delete Records */
DELETE FROM silver_bec_ods.MTL_ONHAND_QUANTITIES
WHERE
  (
    ...
    FROM silver_bec_ods.MTL_ONHAND_QUANTITIES AS ods, bronze_bec_ods_stg.MTL_ONHAND_QUANTITIES AS stg
    WHERE
      COALESCE(ods.ONHAND_QUANTITIES_ID, 0) = COALESCE(stg.ONHAND_QUANTITIES_ID, 0)
      AND stg.kca_operation IN ('INSERT', 'UPDATE')
  );
/* Insert records */
INSERT INTO silver_bec_ods.MTL_ONHAND_QUANTITIES (
  inventory_item_id,
  organization_id,
  date_received,
  last_update_date,
...
(
  SELECT
    ...
    kca_seq_date
  FROM bronze_bec_ods_stg.MTL_ONHAND_QUANTITIES
  WHERE
...
/* Soft delete */
UPDATE silver_bec_ods.MTL_ONHAND_QUANTITIES SET IS_DELETED_FLG = 'N';
UPDATE silver_bec_ods.MTL_ONHAND_QUANTITIES SET IS_DELETED_FLG = 'Y'
WHERE
  (
    ONHAND_QUANTITIES_ID
  ) IN (
