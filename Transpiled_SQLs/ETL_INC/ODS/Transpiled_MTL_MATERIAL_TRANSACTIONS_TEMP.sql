/* Delete Records */
DELETE FROM silver_bec_ods.MTL_MATERIAL_TRANSACTIONS_TEMP
WHERE
  (COALESCE(TRANSACTION_HEADER_ID, 0), COALESCE(TRANSACTION_TEMP_ID, 0)) IN (
    SELECT
      COALESCE(stg.TRANSACTION_HEADER_ID, 0) AS TRANSACTION_HEADER_ID,
      COALESCE(stg.TRANSACTION_TEMP_ID, 0) AS TRANSACTION_TEMP_ID
    FROM silver_bec_ods.MTL_MATERIAL_TRANSACTIONS_TEMP AS ods, bronze_bec_ods_stg.MTL_MATERIAL_TRANSACTIONS_TEMP AS stg
    WHERE
      COALESCE(ods.TRANSACTION_HEADER_ID, 0) = COALESCE(stg.TRANSACTION_HEADER_ID, 0)
      AND COALESCE(ods.TRANSACTION_TEMP_ID, 0) = COALESCE(stg.TRANSACTION_TEMP_ID, 0)
...
(
  ...
  WHERE
    ...
    AND (COALESCE(TRANSACTION_HEADER_ID, 0), COALESCE(TRANSACTION_TEMP_ID, 0), KCA_SEQ_ID) IN (
      SELECT
...
/* Soft delete */
UPDATE silver_bec_ods.MTL_MATERIAL_TRANSACTIONS_TEMP SET IS_DELETED_FLG = 'N';
UPDATE silver_bec_ods.MTL_MATERIAL_TRANSACTIONS_TEMP SET IS_DELETED_FLG = 'Y'
WHERE
  (COALESCE(TRANSACTION_HEADER_ID, 0), COALESCE(TRANSACTION_TEMP_ID, 0)) IN (
    SELECT
      COALESCE(TRANSACTION_HEADER_ID, 0),
    ...
    WHERE
      (COALESCE(TRANSACTION_HEADER_ID, 0), COALESCE(TRANSACTION_TEMP_ID, 0), KCA_SEQ_ID) IN (
        SELECT
          COALESCE(TRANSACTION_HEADER_ID, 0),
          COALESCE(TRANSACTION_TEMP_ID, 0),
