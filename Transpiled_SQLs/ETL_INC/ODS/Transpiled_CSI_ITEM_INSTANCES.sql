/* Delete Records */
DELETE FROM silver_bec_ods.csi_item_instances
WHERE
  (
    COALESCE(INSTANCE_ID, 0)
  ) IN (
    SELECT
      COALESCE(stg.INSTANCE_ID, 0) AS INSTANCE_ID
    FROM silver_bec_ods.csi_item_instances AS ods, bronze_bec_ods_stg.csi_item_instances AS stg
    WHERE
      COALESCE(ods.INSTANCE_ID, 0) = COALESCE(stg.INSTANCE_ID, 0)
...
/* Soft delete */
UPDATE silver_bec_ods.csi_item_instances SET IS_DELETED_FLG = 'N';
UPDATE silver_bec_ods.csi_item_instances SET IS_DELETED_FLG = 'Y'
WHERE
  (
    INSTANCE_ID
  ) IN (
