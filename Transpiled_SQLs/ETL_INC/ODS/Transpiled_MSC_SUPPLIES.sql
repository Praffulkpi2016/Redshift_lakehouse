/* Delete Records */
DELETE FROM silver_bec_ods.MSC_SUPPLIES
WHERE
  (COALESCE(PLAN_ID, 0), COALESCE(SR_INSTANCE_ID, 0), COALESCE(TRANSACTION_ID, 0)) IN (
    SELECT
    ...
    FROM silver_bec_ods.MSC_SUPPLIES AS ods, bronze_bec_ods_stg.MSC_SUPPLIES AS stg
    WHERE
      COALESCE(ods.PLAN_ID, 0) = COALESCE(stg.PLAN_ID, 0)
      AND COALESCE(ods.SR_INSTANCE_ID, 0) = COALESCE(stg.SR_INSTANCE_ID, 0)
      AND COALESCE(ods.TRANSACTION_ID, 0) = COALESCE(stg.TRANSACTION_ID, 0)
...
/* Soft delete */
UPDATE silver_bec_ods.MSC_SUPPLIES SET IS_DELETED_FLG = 'N';
UPDATE silver_bec_ods.MSC_SUPPLIES SET IS_DELETED_FLG = 'Y'
WHERE
  (COALESCE(PLAN_ID, 0), COALESCE(SR_INSTANCE_ID, 0), COALESCE(TRANSACTION_ID, 0)) IN (
    SELECT
      COALESCE(PLAN_ID, 0),
    ...
    WHERE
      (COALESCE(PLAN_ID, 0), COALESCE(SR_INSTANCE_ID, 0), COALESCE(TRANSACTION_ID, 0), KCA_SEQ_ID) IN (
        SELECT
          COALESCE(PLAN_ID, 0),
          COALESCE(SR_INSTANCE_ID, 0),
          COALESCE(TRANSACTION_ID, 0),
