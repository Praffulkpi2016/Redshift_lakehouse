/* Delete Records */
DELETE FROM silver_bec_ods.ap_invoices_all
WHERE
  INVOICE_ID IN (
    SELECT
      stg.INVOICE_ID
    FROM silver_bec_ods.ap_invoices_all AS ods, bronze_bec_ods_stg.ap_invoices_all AS stg
    WHERE
      ods.INVOICE_ID = stg.INVOICE_ID AND stg.kca_operation IN ('INSERT', 'UPDATE')
  );
/* Insert records */
...
/* Soft delete */
UPDATE silver_bec_ods.ap_invoices_all SET IS_DELETED_FLG = 'N';
UPDATE silver_bec_ods.ap_invoices_all SET IS_DELETED_FLG = 'Y'
WHERE
  (
    INVOICE_ID
  ) IN (
