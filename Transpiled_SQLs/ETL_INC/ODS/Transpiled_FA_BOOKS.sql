/* Delete Records */
DELETE FROM silver_bec_ods.FA_BOOKS
WHERE
  (
    ...
    FROM silver_bec_ods.FA_BOOKS AS ods, bronze_bec_ods_stg.FA_BOOKS AS stg
    WHERE
      COALESCE(ods.TRANSACTION_HEADER_ID_IN, 0) = COALESCE(stg.TRANSACTION_HEADER_ID_IN, 0)
...
(
  ...
  WHERE
    ...
    AND (COALESCE(TRANSACTION_HEADER_ID_IN, 0), kca_seq_id) IN (
      SELECT
...
/* Soft delete */
UPDATE silver_bec_ods.FA_BOOKS SET IS_DELETED_FLG = 'N';
UPDATE silver_bec_ods.FA_BOOKS SET IS_DELETED_FLG = 'Y'
WHERE
  (
    TRANSACTION_HEADER_ID_IN
  ) IN (
    ...
    WHERE
      (TRANSACTION_HEADER_ID_IN, KCA_SEQ_ID) IN (
        SELECT
          TRANSACTION_HEADER_ID_IN,
