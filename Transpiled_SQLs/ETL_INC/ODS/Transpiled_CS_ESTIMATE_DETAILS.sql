/* Delete Records */
DELETE FROM silver_bec_ods.CS_ESTIMATE_DETAILS
WHERE
  (
    COALESCE(ESTIMATE_DETAIL_ID, 0)
  ) IN (
    SELECT
      COALESCE(stg.ESTIMATE_DETAIL_ID, 0) AS ESTIMATE_DETAIL_ID
    FROM silver_bec_ods.CS_ESTIMATE_DETAILS AS ods, bronze_bec_ods_stg.CS_ESTIMATE_DETAILS AS stg
    WHERE
      COALESCE(ods.ESTIMATE_DETAIL_ID, 0) = COALESCE(stg.ESTIMATE_DETAIL_ID, 0)
...
(
  ...
  WHERE
    ...
    AND (COALESCE(ESTIMATE_DETAIL_ID, 0), kca_seq_id) IN (
      SELECT
...
/* Soft delete */
UPDATE silver_bec_ods.CS_ESTIMATE_DETAILS SET IS_DELETED_FLG = 'N';
UPDATE silver_bec_ods.CS_ESTIMATE_DETAILS SET IS_DELETED_FLG = 'Y'
WHERE
  (
    ESTIMATE_DETAIL_ID
  ) IN (
    ...
    WHERE
      (ESTIMATE_DETAIL_ID, KCA_SEQ_ID) IN (
        SELECT
          ESTIMATE_DETAIL_ID,
