/* Delete Records */
DELETE FROM silver_bec_ods.pa_expenditure_items_all
WHERE
  (
    COALESCE(EXPENDITURE_ITEM_ID, 0)
  ) IN (
    SELECT
      COALESCE(stg.EXPENDITURE_ITEM_ID, 0) AS EXPENDITURE_ITEM_ID
    FROM silver_bec_ods.pa_expenditure_items_all AS ods, bronze_bec_ods_stg.pa_expenditure_items_all AS stg
    WHERE
      COALESCE(ods.EXPENDITURE_ITEM_ID, 0) = COALESCE(stg.EXPENDITURE_ITEM_ID, 0)
...
(
  ...
  WHERE
    ...
    AND (COALESCE(EXPENDITURE_ITEM_ID, 0), kca_seq_id) IN (
      SELECT
...
/* Soft delete */
UPDATE silver_bec_ods.pa_expenditure_items_all SET IS_DELETED_FLG = 'N';
UPDATE silver_bec_ods.pa_expenditure_items_all SET IS_DELETED_FLG = 'Y'
WHERE
  (
    EXPENDITURE_ITEM_ID
  ) IN (
    ...
    WHERE
      (EXPENDITURE_ITEM_ID, KCA_SEQ_ID) IN (
        SELECT
          EXPENDITURE_ITEM_ID,
