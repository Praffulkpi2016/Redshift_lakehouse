/* delete */
DELETE FROM gold_bec_dwh.FACT_OM_CUSTOMER
WHERE
  EXISTS(
    SELECT
      1
    FROM gold_bec_dwh.DIM_CUSTOMER_DETAILS AS BCDV, silver_bec_ods.HZ_PARTY_SITES AS HPS, silver_bec_ods.HZ_LOCATIONS AS HL, silver_bec_ods.HZ_CUST_ACCOUNTS AS HCAL, silver_bec_ods.HZ_CUST_ACCOUNTS AS HCA, silver_bec_ods.HZ_PARTIES AS HP, silver_bec_ods.HZ_CUST_SITE_USES_ALL AS HCSU, silver_bec_ods.HZ_CUSTOMER_PROFILES AS HCP, silver_bec_ods.HZ_CUST_PROFILE_CLASSES AS HCPC
    WHERE
      1 = 1
      ...
      AND FACT_OM_CUSTOMER.dw_load_id = (
        SELECT
          system_id
...
/* insert */
INSERT INTO gold_bec_dwh.FACT_OM_CUSTOMER
(
  SELECT
    BCDV.PARTY_SITE_ID,
