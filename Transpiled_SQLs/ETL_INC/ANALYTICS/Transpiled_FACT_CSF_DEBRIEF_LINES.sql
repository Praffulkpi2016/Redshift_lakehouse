/* DELETE */
DELETE FROM gold_bec_dwh.fact_csf_debrief_lines
WHERE
  EXISTS(
    SELECT
      1
    FROM silver_bec_ods.csf_debrief_lines AS cdh
    WHERE
      cdh.kca_seq_date > (
        SELECT
          (
      ...
      AND COALESCE(fact_csf_debrief_lines.debrief_header_id, 0) = COALESCE(cdh.debrief_header_id, 0)
      AND COALESCE(fact_csf_debrief_lines.debrief_line_id, 0) = COALESCE(cdh.debrief_line_id, 0)
  );
INSERT INTO gold_bec_dwh.FACT_CSF_DEBRIEF_LINES
SELECT
  cdl.channel_code,
  cdl.debrief_line_id,
  ...
  (
    ...
    WHERE
      source_system = 'EBS'
  ) || '-' || COALESCE(cdl.debrief_line_id, 0) AS dw_load_id,
