/* Delete Records */
DELETE FROM gold_bec_dwh.FACT_PO_SHIPMENT_STG
WHERE
  (COALESCE(vendor_id, 0), COALESCE(SHIPMENT_HEADER_ID, 0), COALESCE(SHIPMENT_LINE_ID, '0'), COALESCE(routing_header_id, '0')) IN (
    SELECT
      COALESCE(ods.vendor_id, 0) AS vendor_id,
      COALESCE(ods.SHIPMENT_HEADER_ID, 0) AS SHIPMENT_HEADER_ID,
      COALESCE(ods.SHIPMENT_LINE_ID, '0') AS SHIPMENT_LINE_ID,
      COALESCE(ods.routing_header_id, '0') AS routing_header_id
    FROM gold_bec_dwh.FACT_PO_SHIPMENT_STG AS dw, (
      SELECT
        rsh.vendor_id,
        RSL.SHIPMENT_HEADER_ID,
        RSL.SHIPMENT_LINE_ID,
        rsl.routing_header_id
      FROM silver_bec_ods.rcv_shipment_headers AS rsh, silver_bec_ods.rcv_shipment_lines AS rsl, silver_bec_ods.fnd_lookup_values AS polc, silver_bec_ods.fnd_lookup_values AS polc2, silver_bec_ods.po_requisition_lines_all AS reql, silver_bec_ods.po_requisition_headers_all AS reqh, silver_bec_ods.rcv_routing_headers AS rrh, silver_bec_ods.mtl_transaction_reasons AS mtr, silver_bec_ods.po_hazard_classes_tl AS pohc, silver_bec_ods.po_un_numbers_tl AS poun, silver_bec_ods.mtl_system_items_b AS msi, silver_bec_ods.hr_locations_all_tl AS hl, silver_bec_ods.per_all_people_f AS he
      WHERE
        1 = 1
      ...
      SELECT
        ...
        RSL.SHIPMENT_LINE_ID,
        rsl.routing_header_id
      FROM silver_bec_ods.rcv_shipment_headers AS rsh, silver_bec_ods.rcv_shipment_lines AS rsl, silver_bec_ods.fnd_lookup_values AS polc, silver_bec_ods.fnd_lookup_values AS polc2, silver_bec_ods.po_headers_all AS poh, silver_bec_ods.po_lines_all AS pol, silver_bec_ods.po_line_locations_all AS poll, silver_bec_ods.po_releases_all AS por, silver_bec_ods.po_vendors AS pov, silver_bec_ods.ap_supplier_sites_all AS povs, silver_bec_ods.rcv_routing_headers AS rrh, silver_bec_ods.mtl_transaction_reasons AS mtr, silver_bec_ods.po_hazard_classes_tl AS pohc, silver_bec_ods.po_un_numbers_tl AS poun, silver_bec_ods.hr_locations_all_tl AS hl, silver_bec_ods.per_all_people_f AS he
      WHERE
        1 = 1
    ...
    WHERE
      dw.dw_load_id = (
        ...
        WHERE
          source_system = 'EBS'
      ) || '-' || COALESCE(ods.vendor_id, 0) || '-' || COALESCE(ods.SHIPMENT_HEADER_ID, 0) || '-' || COALESCE(ods.SHIPMENT_LINE_ID, '0') || '-' || COALESCE(ods.routing_header_id, '0')
  );
/* Insert records */
INSERT INTO gold_bec_dwh.FACT_PO_SHIPMENT_STG (
  SOURCE_TYPE,
  CREATED_BY,
  CREATION_DATE,
  LAST_UPDATED_BY,
...
(
  ...
  FROM (
    ...
    SELECT
      ...
      rsl.destination_context,
      rsl.destination_type_code,
      polc.meaning AS destination_type, /* polc.po_type_display    destination_type, */
      rsl.employee_id,
      rsl.from_organization_id,
      rsl.item_description,
      rsl.item_id,
