/* Delete Records */
DROP table IF EXISTS gold_bec_dwh.FACT_WO_VALUE_VARIANCE_UNION3_STG_Temp;
CREATE TABLE gold_bec_dwh.FACT_WO_VALUE_VARIANCE_UNION3_STG_Temp AS
(
  SELECT
    organization_id,
    (
...
DELETE FROM gold_bec_dwh.FACT_WO_VALUE_VARIANCE_UNION3_STG
WHERE
  (COALESCE(organization_id, 0), COALESCE(CAST(inventory_item_id AS INT), 0), COALESCE(wip_entity_id, 0), COALESCE(CAST(operation_seq_num AS INT), 0), COALESCE(CAST(res_op_seq_num AS INT), 0), COALESCE(CAST(resource_seq_num AS INT), 0), COALESCE(WIP_Supply_Type, 'NA'), COALESCE(ALTERNATE_BOM, 'NA'), COALESCE(comments, 'NA')) IN (
    ...
    FROM gold_bec_dwh.FACT_WO_VALUE_VARIANCE_UNION3_STG AS dw, (
      SELECT
        *
      FROM gold_bec_dwh.FACT_WO_VALUE_VARIANCE_UNION3_STG_Temp
    ) AS ods
    WHERE
      1 = 1
      AND dw.dw_load_id = (
...
/* Insert Records */
INSERT INTO gold_bec_dwh.FACT_WO_VALUE_VARIANCE_UNION3_STG (
  organization_id,
  organization_id_key,
  primary_item_id,
  inventory_item_id,
