/* Delete Records */
DROP table IF EXISTS gold_bec_dwh.FACT_WO_VALUE_VARIANCE_UNION1_STG_INCR;
CREATE TABLE gold_bec_dwh.FACT_WO_VALUE_VARIANCE_UNION1_STG_INCR AS
(
  /* ----------------------------------------------------------------------------------	*/
  WITH wro AS (
    SELECT
...
/* --------------------------------------------------------------------------- */ /* --------------------------------------------------------------------------- */
DELETE FROM gold_bec_dwh.FACT_WO_VALUE_VARIANCE_UNION1_STG
WHERE
  (COALESCE(organization_id, 0), COALESCE(inventory_item_id, 0), COALESCE(wip_entity_id, 0), COALESCE(CAST(operation_seq_num AS INT), 0), COALESCE(CAST(res_op_seq_num AS INT), 0), COALESCE(CAST(resource_seq_num AS INT), 0), COALESCE(WIP_Supply_Type, 'NA'), COALESCE(ALTERNATE_BOM, 'NA'), COALESCE(comments, 'NA')) IN (
    SELECT
      COALESCE(ods.organization_id, 0),
      COALESCE(ods.inventory_item_id, 0),
      COALESCE(ods.wip_entity_id, 0),
      COALESCE(CAST(ods.operation_seq_num AS INT), 0),
      COALESCE(CAST(ods.res_op_seq_num AS INT), 0),
      COALESCE(CAST(ods.resource_seq_num AS INT), 0),
      COALESCE(ods.WIP_Supply_Type, 'NA'),
      COALESCE(ods.ALTERNATE_BOM, 'NA'),
      COALESCE(ods.comments, 'NA')
    FROM gold_bec_dwh.FACT_WO_VALUE_VARIANCE_UNION1_STG_INCR AS ods, gold_bec_dwh.FACT_WO_VALUE_VARIANCE_UNION1_STG AS dw
    WHERE
      1 = 1
      AND dw.dw_load_id = (
        SELECT
        ...
        WHERE
          source_system = 'EBS'
      ) || '-' || COALESCE(ods.organization_id, 0) || '-' || COALESCE(ods.inventory_item_id, 0) || '-' || COALESCE(ods.wip_entity_id, 0) || '-' || COALESCE(CAST(ods.operation_seq_num AS INT), 0) || '-' || COALESCE(CAST(ods.res_op_seq_num AS INT), 0) || '-' || COALESCE(CAST(ods.resource_seq_num AS INT), 0) || '-' || COALESCE(ods.WIP_Supply_Type, 'NA') || '-' || COALESCE(ods.ALTERNATE_BOM, 'NA') || '-' || COALESCE(ods.comments, 'NA')
  );
/* --------------------------------------------------------------------------- */ /* --------------------------------------------------------------------------- */
INSERT INTO gold_bec_dwh.FACT_WO_VALUE_VARIANCE_UNION1_STG
SELECT
  *
FROM gold_bec_dwh.FACT_WO_VALUE_VARIANCE_UNION1_STG_INCR;
