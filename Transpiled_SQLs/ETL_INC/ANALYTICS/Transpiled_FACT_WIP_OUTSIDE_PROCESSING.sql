/* Delete Records */
DELETE FROM gold_bec_dwh.FACT_WIP_OUTSIDE_PROCESSING
WHERE
  (
    ...
    FROM gold_bec_dwh.FACT_WIP_OUTSIDE_PROCESSING AS dw, (
      SELECT
        POD.PO_DISTRIBUTION_ID
      FROM silver_bec_ods.WIP_DISCRETE_JOBS AS DJ, silver_bec_ods.WIP_ENTITIES AS WE, silver_bec_ods.MTL_SYSTEM_ITEMS_B AS SI2, silver_bec_ods.FND_LOOKUP_VALUES AS LU1, silver_bec_ods.FND_LOOKUP_VALUES AS LU2, silver_bec_ods.PO_HEADERS_ALL AS POH, silver_bec_ods.MTL_UNITS_OF_MEASURE_TL AS UOM, silver_bec_ods.PO_LINES_ALL AS POL, silver_bec_ods.PO_LINE_LOCATIONS_ALL AS POLL, silver_bec_ods.PO_DISTRIBUTIONS_ALL AS POD, silver_bec_ods.AP_SUPPLIERS AS APS, silver_bec_ods.PO_RELEASES_ALL AS POR, (
        SELECT
          ORG.ORGANIZATION_ID,
          P.PERSON_ID AS EMPLOYEE_ID,
          P.FULL_NAME,
          p.is_deleted_flg,
          ORG.is_deleted_flg AS is_deleted_flg1
        FROM silver_bec_ods.PER_ALL_PEOPLE_F AS P, silver_bec_ods.HR_ALL_ORGANIZATION_UNITS AS ORG
        WHERE
      ...
      ) AS MEV, silver_bec_ods.BOM_RESOURCES AS BR, silver_bec_ods.MTL_SYSTEM_ITEMS_B AS SI1, silver_bec_ods.HR_ORGANIZATION_INFORMATION AS OOG
      WHERE
        1 = 1
    ...
    WHERE
      ...
      AND dw.dw_load_id = (
        ...
        WHERE
          source_system = 'EBS'
      ) || '-' || COALESCE(ods.PO_DISTRIBUTION_ID, 0)
  );
/* Insert Records */
INSERT INTO gold_bec_dwh.FACT_WIP_OUTSIDE_PROCESSING (
  WIP_ENTITY_ID,
  PRIMARY_ITEM_ID,
  ORGANIZATION_ID,
  INVENTORY_ITEM_ID,
