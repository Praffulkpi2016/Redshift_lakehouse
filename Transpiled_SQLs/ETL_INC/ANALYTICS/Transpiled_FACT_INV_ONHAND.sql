TRUNCATE table gold_bec_dwh.FACT_INV_ONHAND_TMP;
INSERT INTO gold_bec_dwh.FACT_INV_ONHAND_TMP
(
  SELECT DISTINCT
    inventory_item_id,
    organization_id
  FROM (
    SELECT
      ...
      miq.organization_id
    FROM silver_bec_ods.mtl_system_items_b AS msi, silver_bec_ods.mtl_onhand_quantities_detail AS miq, silver_bec_ods.mtl_material_transactions AS mmt, silver_bec_ods.mtl_transaction_types AS mtt, silver_bec_ods.cst_item_costs AS ctc, silver_bec_ods.rcv_transactions AS rct, silver_bec_ods.mtl_txn_source_types AS mts, silver_bec_ods.org_organization_definitions AS ood, silver_bec_ods.mtl_secondary_inventories AS sub
    WHERE
    ...
    SELECT
      ...
      msi.organization_id
    FROM silver_bec_ods.mtl_system_items_b AS msi, silver_bec_ods.mtl_serial_numbers AS msn, silver_bec_ods.mtl_material_transactions AS mmt, silver_bec_ods.mtl_unit_transactions AS mut, silver_bec_ods.mtl_transaction_types AS mtt, silver_bec_ods.cst_item_costs AS ctc, silver_bec_ods.rcv_transactions AS rct, silver_bec_ods.mtl_txn_source_types AS mts, silver_bec_ods.org_organization_definitions AS ood, silver_bec_ods.mtl_secondary_inventories AS sub
    WHERE
...
DELETE FROM gold_bec_dwh.FACT_INV_ONHAND
WHERE
  (inventory_item_id, organization_id) IN (
    SELECT
      inventory_item_id,
      organization_id
    FROM gold_bec_dwh.FACT_INV_ONHAND_TMP
  );
/* INSERT */
INSERT INTO gold_bec_dwh.FACT_INV_ONHAND
SELECT DISTINCT
  TRANSACTION_ID,
  inventory_item_id,
  organization_id,
...
FROM (
  ...
  FROM (
    ...
    WHERE
      is_deleted_flg <> 'Y'
  ) AS sub, gold_bec_dwh.FACT_INV_ONHAND_TMP AS tmp
  WHERE
