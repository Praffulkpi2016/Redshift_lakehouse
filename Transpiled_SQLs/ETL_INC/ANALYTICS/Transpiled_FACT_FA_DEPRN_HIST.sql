/* Delete Records */
DELETE FROM gold_bec_dwh.FACT_FA_DEPRN_HIST
WHERE
  (COALESCE(asset_id, 0), COALESCE(distribution_id, 0), COALESCE(book_type_code, 'NA'), COALESCE(period_name, 'NA')) IN (
    SELECT
    ...
    FROM gold_bec_dwh.FACT_FA_DEPRN_HIST AS dw, (
      SELECT
        asset_id,
        book_type_code,
        distribution_id,
      ...
      FROM (
        (
          ...
          SELECT
            ...
            dp.period_name
          FROM silver_bec_ods.fa_deprn_detail AS dd_bonus
          INNER JOIN silver_bec_ods.fa_category_books AS cb
            ON dd_bonus.book_type_code = cb.book_type_code
          INNER JOIN silver_bec_ods.fa_distribution_history AS dh
            ON dd_bonus.distribution_id = dh.distribution_id
          INNER JOIN silver_bec_ods.fa_books AS books
            ON cb.book_type_code = books.book_type_code AND dh.asset_id = books.asset_id
          INNER JOIN silver_bec_ods.fa_asset_history AS ah
          ...
          INNER JOIN (
            SELECT
              ...
              bc.book_type_code,
              dp.is_deleted_flg,
              dp_fy.is_deleted_flg AS is_deleted_flg1,
              bc.is_deleted_flg AS is_deleted_flg2
            FROM silver_bec_ods.fa_deprn_periods AS dp, silver_bec_ods.fa_deprn_periods AS dp_fy, silver_bec_ods.fa_book_controls AS bc
            WHERE
            ...
            AND dd_bonus.period_counter = (
              ...
              FROM silver_bec_ods.fa_deprn_detail AS dd_sub
              WHERE
          ...
          INNER JOIN silver_bec_ods.fa_transaction_headers AS th_rt
            ON cb.book_type_code = th_rt.book_type_code
            AND books.transaction_header_id_in = th_rt.transaction_header_id
          LEFT OUTER JOIN silver_bec_ods.fa_transaction_headers AS th
          ...
          LEFT OUTER JOIN silver_bec_ods.fa_deprn_summary AS ds
        ...
        (
          ...
          SELECT
            ...
            dd.distribution_id,
            dp.period_name
          FROM silver_bec_ods.fa_deprn_detail AS dd
          INNER JOIN silver_bec_ods.fa_category_books AS cb
    ...
    WHERE
      ...
      AND dw.dw_load_id = (
        ...
        WHERE
          source_system = 'EBS'
      ) || '-' || COALESCE(ods.asset_id, 0) || '-' || COALESCE(ods.distribution_id, 0) || '-' || COALESCE(ods.book_type_code, 'NA') || '-' || COALESCE(ods.period_name, 'NA')
  );
/* Insert Records */
WITH cte_asgn_loc AS (
  SELECT
...
INSERT INTO gold_bec_dwh.FACT_FA_DEPRN_HIST (
  asset_id,
  book_type_code,
  dh_ccid,
  deprn_reserve_acct,
