/* Delete Records */
DELETE FROM gold_bec_dwh.FACT_WO_VALUE_VARIANCE_UNION5_STG
WHERE
  (COALESCE(organization_id, 0), COALESCE(CAST(inventory_item_id AS INT), 0), COALESCE(wip_entity_id, 0), COALESCE(CAST(operation_seq_num AS INT), 0), COALESCE(CAST(res_op_seq_num AS INT), 0), COALESCE(CAST(resource_seq_num AS INT), 0), COALESCE(WIP_Supply_Type, 'NA'), COALESCE(ALTERNATE_BOM, 'NA'), COALESCE(comments, 'NA')) IN (
    SELECT
      ...
      COALESCE(CAST(ods.inventory_item_id AS INT), 0),
    ...
    FROM gold_bec_dwh.FACT_WO_VALUE_VARIANCE_UNION5_STG AS dw, (
      SELECT
        organization_id,
        (
          SELECT
      ...
      FROM (
        SELECT
          ...
          (
            ...
            FROM silver_bec_ods.WIP_TRANSACTION_ACCOUNTS AS a, silver_bec_ods.wip_transactions AS t
            WHERE
          ...
          (
            ...
            WHERE
              ...
              AND t.wip_entity_id = e.wip_entity_id
              AND t.transaction_type = 6
              AND COALESCE(A.COST_ELEMENT_ID, -999) = -999
              AND a.accounting_line_type IN (7, 8)
          ) AS net_value,
          d.organization_id AS organization_id1,
          NULL AS ALTERNATE_BOM,
          'Work Order Variance' AS COMMENTS
        FROM silver_bec_ods.wip_entities AS e, silver_bec_ods.wip_discrete_jobs AS d
        WHERE
...
/* -------------------------------------------------------------------------------------- */ /* -------------------------------------------------------------------------------------- */
INSERT INTO gold_bec_dwh.fact_wo_value_variance_union5_stg (
  organization_id,
  organization_id_key,
  primary_item_id,
  inventory_item_id,
