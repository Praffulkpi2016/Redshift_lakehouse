/* Delete Records */
DELETE FROM gold_bec_dwh.FACT_TRIAL_BALANCE
WHERE
  (COALESCE(ledger_id, 0), COALESCE(currency_code, 'NA'), COALESCE(period_set_name, 'NA'), COALESCE(period_name, 'NA'), COALESCE(actual_flag, 'NA'), COALESCE(code_combination_id, 0), COALESCE(translated_flag, 'x'), COALESCE(budget_version_id, 0)) IN (
    SELECT
    ...
    ) AS ods, gold_bec_dwh.FACT_TRIAL_BALANCE AS dw
    WHERE
      dw.dw_load_id = (
        SELECT
          system_id
        ...
        WHERE
          source_system = 'EBS'
      ) || '-' || COALESCE(ods.ledger_id, 0) || '-' || COALESCE(ods.period_set_name, 'NA') || '-' || COALESCE(ods.currency_code, 'NA') || '-' || COALESCE(ods.period_name, 'NA') || '-' || COALESCE(ods.actual_flag, 'NA') || '-' || COALESCE(ods.code_combination_id, 0) || '-' || COALESCE(ods.translated_flag, 'x') || '-' || COALESCE(ods.budget_version_id, 0)
  );
/* Insert records */
INSERT INTO gold_bec_dwh.FACT_TRIAL_BALANCE (
  ledger_id,
  period_set_name,
  currency_code,
  period_name,
...
(
  SELECT
    ...
    (
      ...
      WHERE
        source_system = 'EBS'
    ) || '-' || COALESCE(ledger_id, 0) || '-' || COALESCE(period_set_name, 'NA') || '-' || COALESCE(currency_code, 'NA') || '-' || COALESCE(period_name, 'NA') || '-' || COALESCE(actual_flag, 'NA') || '-' || COALESCE(code_combination_id, 0) || '-' || COALESCE(translated_flag, 'x') || '-' || COALESCE(budget_version_id, 0) AS dw_load_id,
  ...
  FROM (
    SELECT
      ...
      ) /* ((gb.begin_balance_dr - gb.begin_balance_cr) - (gb.period_net_dr - gb.period_net_cr)) as ending_balance, */ /*	(gb.begin_balance_dr-gb.begin_balance_cr) - (gb.period_net_cr_beq-gb.period_net_dr_beq) Ending_Balance, */ + (
        gb.period_net_dr - gb.period_net_cr
      ) AS Ending_Balance,
      (
        gb.begin_balance_dr - gb.begin_balance_cr
