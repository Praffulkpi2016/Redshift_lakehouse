TRUNCATE table gold_bec_dwh.fact_rtv_serial_dtls_tmp;
INSERT INTO gold_bec_dwh.fact_rtv_serial_dtls_tmp
SELECT DISTINCT
  msi.inventory_item_id,
  mmt.organization_id,
  mut.serial_number
FROM silver_bec_ods.mtl_material_transactions AS mmt, silver_bec_ods.mtl_unit_transactions AS mut, silver_bec_ods.mtl_system_items_b AS msi, silver_bec_ods.mtl_serial_numbers AS msn
WHERE
  1 = 1
  AND (
    mmt.kca_seq_date > (
...
DELETE FROM gold_bec_dwh.fact_rtv_serial_dtls
WHERE
  EXISTS(
    SELECT
      1
    FROM gold_bec_dwh.fact_rtv_serial_dtls_tmp AS tmp
    WHERE
      1 = 1
...
INSERT INTO gold_bec_dwh.FACT_RTV_SERIAL_DTLS
(
  SELECT
  ...
  FROM (
    (
      ...
      INNER JOIN gold_bec_dwh.fact_rtv_serial_dtls_tmp AS tmp
