/* Delete Records */
DELETE FROM gold_bec_dwh.FACT_XLA_REPORT_BALANCES_STG
WHERE
  (COALESCE(LEDGER_ID, 0), COALESCE(BUDGET_NAME, 'NA'), COALESCE(PERIOD_NAME, 'NA'), COALESCE(CODE_COMBINATION_ID, 0), COALESCE(LEDGER_CURRENCY, 'NA'), COALESCE(TRANSLATED_FLAG, 'NA')) IN (
    SELECT
      COALESCE(ods.LEDGER_ID, 0) AS LEDGER_ID,
      COALESCE(ods.BUDGET_NAME, 'NA') AS BUDGET_NAME,
    ...
    FROM gold_bec_dwh.FACT_XLA_REPORT_BALANCES_STG AS dw, (
      SELECT
      ...
      FROM (
        SELECT
          ...
          gls.start_date,
          gls.end_date
        FROM silver_bec_ods.gl_ledgers AS gll, silver_bec_ods.gl_period_statuses AS gls
        WHERE
          gls.ledger_id = gll.ledger_id
          ...
          AND (
            ...
            OR gll.IS_DELETED_FLG = 'Y'
            OR gls.IS_DELETED_FLG = 'Y'
          )
      ) AS gl1, silver_bec_ods.gl_balances AS glb, silver_bec_ods.gl_code_combinations AS gcck, silver_bec_ods.fnd_lookup_values AS xlk, silver_bec_ods.gl_budget_versions AS glv, silver_bec_ods.gl_encumbrance_types AS get
      WHERE
        ...
        AND (
          ...
          OR glb.IS_DELETED_FLG = 'Y'
          OR gcck.IS_DELETED_FLG = 'Y'
          OR xlk.IS_DELETED_FLG = 'Y'
          OR glv.IS_DELETED_FLG = 'Y'
          OR get.IS_DELETED_FLG = 'Y'
        )
    ) AS ods
    WHERE
      1 = 1
      AND dw.dw_load_id = (
        ...
        WHERE
          source_system = 'EBS'
      ) || '-' || COALESCE(ods.LEDGER_ID, 0) || '-' || COALESCE(ods.BUDGET_NAME, 'NA') || '-' || COALESCE(ods.PERIOD_NAME, 'NA') || '-' || COALESCE(ods.CODE_COMBINATION_ID, 0) || '-' || COALESCE(ods.LEDGER_CURRENCY, 'NA') || '-' || COALESCE(ods.TRANSLATED_FLAG, 'NA')
  );
/* Insert Records */
INSERT INTO gold_bec_dwh.FACT_XLA_REPORT_BALANCES_STG (
  LEDGER_ID,
  LEDGER_SHORT_NAME,
  LEDGER_DESCRIPTION,
  LEDGER_NAME,
