TRUNCATE table gold_bec_dwh.fact_wo_serial_genealogy_tmp;
/* Insert records into temp table */
INSERT INTO gold_bec_dwh.fact_wo_serial_genealogy_tmp
(
  SELECT DISTINCT
    we.WIP_ENTITY_ID
  FROM (
...
DELETE FROM gold_bec_dwh.fact_wo_serial_genealogy
WHERE
  EXISTS(
    SELECT
      1
    FROM gold_bec_dwh.fact_wo_serial_genealogy_tmp AS tmp
    WHERE
      COALESCE(tmp.WIP_ENTITY_ID, 0) = COALESCE(fact_wo_serial_genealogy.WIP_ENTITY_ID, 0)
  );
/* Insert records into fact table */
INSERT INTO gold_bec_dwh.fact_wo_serial_genealogy
(
  SELECT
    part_number,
    inventory_item_id,
  ...
  FROM (
    ...
    FROM (
      ...
      WHERE
        IS_DELETED_FLG <> 'Y'
    ) AS we, gold_bec_dwh.fact_wo_serial_genealogy_tmp AS tmp
    WHERE
      mmt.transaction_id = mut.transaction_id
      AND mmt.inventory_item_id = msi.INVENTORY_ITEM_ID()
      AND mmt.organization_Id = msi.ORGANIZATION_ID()
