TRUNCATE table gold_bec_dwh.fact_wip_period_value_tmp;
...
INSERT INTO gold_bec_dwh.fact_wip_period_value_tmp
(
  SELECT DISTINCT
    we.WIP_ENTITY_ID
...
DELETE FROM gold_bec_dwh.fact_wip_period_value
WHERE
  EXISTS(
    SELECT
      1
    FROM gold_bec_dwh.fact_wip_period_value_tmp AS tmp
    WHERE
      COALESCE(tmp.WIP_ENTITY_ID, 0) = COALESCE(fact_wip_period_value.WIP_ENTITY_ID, 0)
  );
...
INSERT INTO gold_bec_dwh.fact_wip_period_value
(
  SELECT
    wdj.organization_id,
  ...
  FROM silver_bec_ods.wip_discrete_jobs AS wdj, silver_bec_ods.wip_entities AS we, silver_bec_ods.mtl_system_items_b AS msi, silver_bec_ods.wip_period_balances AS wpb, silver_bec_ods.org_organization_definitions AS ood, silver_bec_ods.fnd_lookup_values AS ml, silver_bec_ods.wip_schedule_groups AS wsg, silver_bec_ods.fnd_user AS fu, totals, wip_operations AS wo, gold_bec_dwh.fact_wip_period_value_tmp AS tmp
  WHERE
    wdj.wip_entity_id = we.wip_entity_id
    AND wdj.organization_id = we.organization_id
    AND wdj.primary_item_id = msi.INVENTORY_ITEM_ID()
    ...
    AND totals.acct_period_id = wpb.acct_period_id
    AND wdj.wip_entity_id = wo.WIP_ENTITY_ID()
    AND COALESCE(tmp.WIP_ENTITY_ID, 0) = COALESCE(we.WIP_ENTITY_ID, 0)
);
