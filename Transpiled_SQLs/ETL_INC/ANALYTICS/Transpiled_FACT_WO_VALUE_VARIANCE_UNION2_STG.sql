/* Delete Records */
DROP table IF EXISTS gold_bec_dwh.FACT_WO_VALUE_VARIANCE_UNION2_STG_INCR;
CREATE TABLE gold_bec_dwh.FACT_WO_VALUE_VARIANCE_UNION2_STG_INCR AS
/* ---------------------------------------------------------------------------------- */
WITH mta AS (
  SELECT
    transaction_id,
...
/* --------------------------------------------------------------------------- */ /* --------------------------------------------------------------------------- */
DELETE FROM gold_bec_dwh.FACT_WO_VALUE_VARIANCE_UNION2_STG
WHERE
  (COALESCE(organization_id, 0), COALESCE(CAST(inventory_item_id AS INT), 0), COALESCE(wip_entity_id, 0), COALESCE(CAST(operation_seq_num AS INT), 0), COALESCE(CAST(res_op_seq_num AS INT), 0), COALESCE(CAST(resource_seq_num AS INT), 0), COALESCE(WIP_Supply_Type, 'NA'), COALESCE(ALTERNATE_BOM, 'NA'), COALESCE(comments, 'NA')) IN (
    ...
    FROM gold_bec_dwh.FACT_WO_VALUE_VARIANCE_UNION2_STG AS dw, gold_bec_dwh.FACT_WO_VALUE_VARIANCE_UNION2_STG_INCR AS ods
    WHERE
      1 = 1
      AND dw.dw_load_id = (
        SELECT
...
/* --------------------------------------------------------------------------- */ /* --------------------------------------------------------------------------- */
INSERT INTO gold_bec_dwh.FACT_WO_VALUE_VARIANCE_UNION2_STG
SELECT
  *
FROM gold_bec_dwh.FACT_WO_VALUE_VARIANCE_UNION2_STG_INCR;
