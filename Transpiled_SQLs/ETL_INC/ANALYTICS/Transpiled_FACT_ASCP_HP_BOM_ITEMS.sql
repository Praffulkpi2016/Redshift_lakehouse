TRUNCATE table gold_bec_dwh.FACT_ASCP_HP_BOM_ITEMS;
WITH items(plan_id, organization_id, inventory_item_id, using_assembly_id, level, root_assembly, bill_sequence_id) AS (
  ...
  FROM (
    SELECT
      *
    FROM silver_bec_ods.msc_boms
    WHERE
      is_deleted_flg <> 'Y'
  ) AS bom1, (
    SELECT
      *
    FROM silver_bec_ods.msc_system_items
    WHERE
      is_deleted_flg <> 'Y'
  ) AS msi, gold_bec_dwh.fact_ascp_hp_items_stg AS fahp2
  WHERE
  ...
  FROM (
    SELECT
      *
    FROM silver_bec_ods.msc_bom_components
    WHERE
      is_deleted_flg <> 'Y'
  ) AS mbc, (
    SELECT
    ...
    WHERE
      is_deleted_flg <> 'Y'
  ) AS bom2, gold_bec_dwh.fact_ascp_hp_items_stg AS fahp3, items
  WHERE
...
INSERT INTO gold_bec_dwh.fact_ascp_hp_bom_items
(
  SELECT
    *,
    (
