TRUNCATE table gold_bec_dwh.fact_cst_inv_distribution_stg_tmp;
/* Insert records into temp table */
INSERT INTO gold_bec_dwh.fact_cst_inv_distribution_stg_tmp
(
  SELECT DISTINCT
    transaction_id,
    inventory_item_id,
...
DELETE FROM gold_bec_dwh.fact_cst_inv_distribution_stg
WHERE
  EXISTS(
    SELECT
      1
    FROM gold_bec_dwh.fact_cst_inv_distribution_stg_tmp AS tmp
    WHERE
      tmp.transaction_id = fact_cst_inv_distribution_stg.transaction_id
...
INSERT INTO gold_bec_dwh.fact_cst_inv_distribution_stg
(
  SELECT DISTINCT
    REFERENCE_ACCOUNT,
  ...
  FROM (
    ...
    FROM (
      ...
      WHERE
        is_deleted_flg <> 'Y'
    ) AS MP1, gold_bec_dwh.fact_cst_inv_distribution_stg_tmp AS tmp
    WHERE
