TRUNCATE table gold_bec_dwh.wo_temp;
WITH on_hand_qty AS (
  SELECT
...
INSERT INTO gold_bec_dwh.wo_temp
(
  SELECT
    *
  FROM (
...
/* delete records */
DELETE FROM gold_bec_dwh_rpt.FACT_WO_NETTABLE_ONHAND_QTY_RT
WHERE
  dw_load_id IN (
    SELECT
      ods.dw_load_id
    FROM gold_bec_dwh_rpt.FACT_WO_NETTABLE_ONHAND_QTY_RT AS dw, (
      SELECT
        (
          SELECT
            system_id
...
UPDATE gold_bec_dwh_rpt.FACT_WO_NETTABLE_ONHAND_QTY_RT AS RT SET ACTIVE_FLG = 'N'
WHERE
  NOT EXISTS(
    SELECT DISTINCT
      ...
      fio.serial_number,
      fio.lot_number
    FROM gold_bec_dwh.fact_inv_onhand AS fio
    WHERE
      1 = 1
      AND fio.vmi_flag <> 'Y'
      AND COALESCE(RT.part_no, 'NA') = COALESCE(fio.part_number, 'NA')
      AND COALESCE(RT.organization_id, 1) = COALESCE(fio.organization_id, 1)
      AND COALESCE(RT.subinventory, 'NA') = COALESCE(fio.subinventory, 'NA')
      AND COALESCE(RT.locator_id, 1) = COALESCE(fio.locator_id, 1)
      AND COALESCE(RT.serial_number, 'NA') = COALESCE(fio.serial_number, 'NA')
      AND COALESCE(RT.lot_number, 'NA') = COALESCE(fio.lot_number, 'NA')
  );
