DROP table IF EXISTS gold_bec_dwh_rpt.FACT_GL_ACCOUNT_ANALYSIS_RT;
CREATE TABLE gold_bec_dwh_rpt.FACT_GL_ACCOUNT_ANALYSIS_RT AS
(
  SELECT
  ...
  FROM `bec_dwh`.`fact_gl_account_analysis` AS `fact_gl_account_analysis`
  INNER JOIN (
    SELECT
