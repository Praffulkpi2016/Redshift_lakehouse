DROP table IF EXISTS gold_bec_dwh.FACT_RAPA_STG6;
CREATE TABLE gold_bec_dwh.FACT_RAPA_STG6 AS
(
  SELECT
    INVENTORY_ITEM_ID,
    organization_id,
    ...
    FROM (
      SELECT
        ...
        (
          ...
          WHERE
            unit_of_measure = pol.unit_meas_lookup_code AND inventory_item_id = 0
        ) AS NV2,
        (
          SELECT
            conversion_rate
        ...
        (
          ...
          WHERE
            unit_of_measure = msi.primary_unit_of_measure AND inventory_item_id = 0
        ) AS NV4,
        price_override,
