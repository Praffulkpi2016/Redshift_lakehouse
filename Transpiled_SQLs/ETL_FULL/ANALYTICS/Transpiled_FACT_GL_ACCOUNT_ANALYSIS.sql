DROP table IF EXISTS gold_bec_dwh.FACT_GL_ACCOUNT_ANALYSIS;
CREATE TABLE gold_bec_dwh.FACT_GL_ACCOUNT_ANALYSIS AS
(
  SELECT DISTINCT
    A.GL_DATE,
    A.created_by_id,
  ...
  FROM (
    ...
    (
      ...
      WITH gjl_gjh AS (
        SELECT
          ...
          gjl.period_name,
          gjl.created_by,
          gjl.je_line_num AS GL_LINE_NUMBER, /* ,to_char(gjl.effective_date,'YYYY-MM-DD HH:MI:SS') effective_date */
          gjl.effective_date,
        ...
        FROM (
          ...
          WHERE
            is_deleted_flg <> 'Y'
        ) AS gdct /*     WHERE  gdct.last_update_date >= to_date('2010-01-01 12:00:00','YYYY-MM-DD HH:MI:SS') */
      ), gjb_papf /* ----------------------------------------------------------------------------------------------------------- */ AS (
        SELECT
          gjb.je_batch_id,
          gjb.NAME,
    ...
    (
      SELECT
        ...
        glbgt.TRANSLATED_FLAG,
        0 AS TEMP_LINE_NUM
      FROM gold_bec_dwh.FACT_XLA_REPORT_BALANCES_STG AS glbgt /* ,fnd_user                         fdu */, (
        SELECT
          *
        ...
        WHERE
          is_deleted_flg <> 'Y'
      ) AS fsv2 /* ,gl_je_categories_tl              gjct */, (
        SELECT
          *
        FROM silver_bec_ods.gl_je_sources_tl
        WHERE
