DROP table IF EXISTS gold_bec_dwh.FACT_TRIAL_BALANCE;
CREATE TABLE gold_bec_dwh.FACT_TRIAL_BALANCE AS
(
  SELECT
    ledger_id,
    period_set_name,
  ...
  FROM (
    ...
    WHERE
      1 = 1 AND gb.actual_flag = 'A' AND gcc.summary_flag = 'N'
    UNION ALL
    SELECT
      gb.ledger_id AS ledger_id,
      gl.period_set_name AS period_set_name,
      ...
      (
        gb.begin_balance_dr - gb.begin_balance_cr
      ) /* ((gb.begin_balance_dr - gb.begin_balance_cr) - (gb.period_net_dr - gb.period_net_cr)) as ending_balance, */ /* ((gb.begin_balance_dr - gb.begin_balance_cr) - (gb.period_net_dr - gb.period_net_cr)) as ending_balance, */ /* (gb.begin_balance_dr-gb.begin_balance_cr) - (gb.period_net_cr_beq-gb.period_net_dr_beq) Ending_Balance, */ + (
        gb.period_net_dr - gb.period_net_cr
      ) AS Ending_Balance,
      (
        gb.begin_balance_dr - gb.begin_balance_cr
    ...
    INNER JOIN (
      ...
      ON gb.ledger_id = gl.ledger_id
    WHERE
      1 = 1 AND gb.actual_flag = 'B' AND gcc.summary_flag = 'N'
  )
);
