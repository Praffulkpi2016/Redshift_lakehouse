DROP table IF EXISTS gold_bec_dwh.DIM_SEGMENT10_HCY;
CREATE TABLE gold_bec_dwh.DIM_SEGMENT10_HCY AS
(
  SELECT DISTINCT
  ...
  FROM (
    ...
    WHERE
      fifs.id_flex_code = 'GL#' AND fifst.id_flex_code = 'GL#' AND fifs.segment_num = 10
  ) AS fnd
  JOIN (
    SELECT
...
WHERE
  dw_table_name = 'dim_segment10_hcy' AND batch_name = 'gl';