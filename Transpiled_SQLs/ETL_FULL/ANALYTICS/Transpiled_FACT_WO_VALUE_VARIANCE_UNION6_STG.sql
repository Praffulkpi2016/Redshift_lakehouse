DROP table IF EXISTS gold_bec_dwh.FACT_WO_VALUE_VARIANCE_UNION6_STG;
CREATE TABLE gold_bec_dwh.FACT_WO_VALUE_VARIANCE_UNION6_STG AS
/* ---------------------------------------------------------------------------------- */
SELECT
  *,
...
FROM (
  ...
  WITH wro AS (
    ...
    WHERE
      b.obj_name IS NULL AND COALESCE(b.effectivity_control, 1) <= 3
  ), bcb /* ---------------------------------------------------------------------------------- */ AS (
    SELECT
      COMPONENT_ITEM_ID,
      LAST_UPDATE_DATE,
    ...
    WHERE
      obj_name IS NULL AND overlapping_changes IS NULL
  ) /*	AND include_in_cost_rollup = 1 */, csc_max_id2 /* ---------------------------------------------------------------------------------- */ AS (
    SELECT
