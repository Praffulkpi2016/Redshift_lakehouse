DROP table IF EXISTS gold_bec_dwh.DIM_SEGMENT2_HCY;
CREATE TABLE gold_bec_dwh.dim_segment2_hcy AS
(
  SELECT DISTINCT
  ...
  FROM (
    ...
    WHERE
      fifs.id_flex_code = 'GL#' AND fifst.id_flex_code = 'GL#' AND fifs.segment_num = 2
  ) AS fnd
  JOIN (
    SELECT
...
WHERE
  dw_table_name = 'dim_segment2_hcy' AND batch_name = 'gl';