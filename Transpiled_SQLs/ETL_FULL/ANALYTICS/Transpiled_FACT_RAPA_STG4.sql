DROP table IF EXISTS gold_bec_dwh.FACT_RAPA_STG4;
CREATE TABLE gold_bec_dwh.FACT_RAPA_STG4 AS
(
  SELECT
    inventory_item_id,
    organization_id,
        ...
        FROM (
          SELECT
              ...
              COALESCE(
                ...
                COALESCE(
                  (
                    ...
                    WHERE
                      unit_of_measure = r.po_unit_of_measure AND inventory_item_id = 0
                  ),
                  1
                )
              ) / COALESCE(
                ...
                COALESCE(
                  (
                    ...
                    FROM silver_bec_ods.mtl_uom_conversions
                    WHERE
                      unit_of_measure = r.primary_unit_of_measure AND inventory_item_id = 0
                  ),
                  1
                )
              )
            ...
            (
              ...
              WHERE
                uom_code = r.primary_uom_code
            ) AS Decodest1,
            (
              SELECT
                MAX(primary_quantity)
              FROM silver_bec_ods.mtl_material_transactions AS mmt, silver_bec_ods.po_line_locations_all AS poll
              WHERE
          ...
          FROM silver_bec_ods.bec_actual_po_recpt1 AS r, silver_bec_ods.mtl_system_items_b AS msi
          WHERE
