DROP table IF EXISTS bronze_bec_ods_stg.fa_mc_deprn_summary;
CREATE TABLE bronze_bec_ods_stg.fa_mc_deprn_summary AS
(
  SELECT
    *
  ...
  WHERE
    ...
    AND (COALESCE(BOOK_TYPE_CODE, 'NA'), COALESCE(ASSET_ID, 0), COALESCE(PERIOD_COUNTER, 0), COALESCE(SET_OF_BOOKS_ID, 0)) IN (
      SELECT
