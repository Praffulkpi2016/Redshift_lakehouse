DROP TABLE IF EXISTS bronze_bec_ods_stg.fa_deprn_detail;
CREATE TABLE bronze_bec_ods_stg.fa_deprn_detail AS
SELECT
  *
FROM bec_raw_dl_ext.fa_deprn_detail
WHERE
  kca_operation <> 'DELETE'
  AND (COALESCE(BOOK_TYPE_CODE, 'NA'), COALESCE(ASSET_ID, 0), COALESCE(DISTRIBUTION_ID, 0), COALESCE(PERIOD_COUNTER, 0)) IN (
    SELECT
