...
drop table if exists bec_dwh.FACT_AP_INV_DISTRIBUTIONS;

create table bec_dwh.FACT_AP_INV_DISTRIBUTIONS diststyle all sortkey (invoice_id,INVOICE_LINE_NUMBER,
...
select
	...
	(select system_id from bec_etl_ctrl.etlsourceappid where source_system='EBS')||'-'||AID.DIST_CODE_COMBINATION_ID as GL_ACCOUNT_ID_KEY,
	(select system_id from bec_etl_ctrl.etlsourceappid where source_system='EBS')||'-'||AID.invoice_id as INVOICE_ID_KEY,
	AID.INVOICE_ID,
	AID.LINE_TYPE_LOOKUP_CODE,
	AID.PERIOD_NAME,
	(select system_id from bec_etl_ctrl.etlsourceappid where source_system='EBS')||'-'||AID.SET_OF_BOOKS_ID as LEDGER_ID_KEY,
	...
	(select system_id from bec_etl_ctrl.etlsourceappid where source_system='EBS')||'-'||nvl(AIA.BATCH_ID, 0)  as BATCH_ID_KEY,
	AID.AMOUNT,
	AID.BASE_AMOUNT,
	AID.DESCRIPTION,
	AID.FINAL_MATCH_FLAG,
	AID.POSTED_FLAG,
	(select system_id from bec_etl_ctrl.etlsourceappid where source_system='EBS')||'-'||AID.PO_DISTRIBUTION_ID as PO_DISTRIBUTION_ID_KEY,
	...
	(select system_id from bec_etl_ctrl.etlsourceappid where source_system='EBS')||'-'||AID.PROJECT_ID as PROJECT_ID_KEY,
	(select system_id from bec_etl_ctrl.etlsourceappid where source_system='EBS')||'-'||AID.PROJECT_ID||'-'||AID.TASK_ID  as PROJECT_TASK_ID_KEY,	
	(select system_id from bec_etl_ctrl.etlsourceappid where source_system='EBS')||'-'||AID.ORG_ID as ORG_ID_KEY,
    ...
    || '-'
       || nvl(AID.INVOICE_ID, 0)-- ||' -'|| nvl(AILA.LINE_NUMBER,0)
	   || '-' || nvl(AID.INVOICE_LINE_NUMBER, 0)|| '-' || nvl(AID.INVOICE_DISTRIBUTION_ID, 0) as dw_load_id,
