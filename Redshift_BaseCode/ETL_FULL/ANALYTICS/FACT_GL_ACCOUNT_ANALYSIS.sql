...
begin;
drop 
  table if exists bec_dwh.FACT_GL_ACCOUNT_ANALYSIS;
create table bec_dwh.FACT_GL_ACCOUNT_ANALYSIS diststyle all sortkey(
  LINE_NUMBER, HEADER_ID, LEDGER_CURRENCY, 
  TRANSLATED_FLAG, TEMP_LINE_NUM
) as (
  SELECT Distinct
    A.GL_DATE, 
    A.created_by_id, 
          ...
          From 
            (
              select 
                * 
              from 
                bec_ods.ap_invoice_distributions_all 
              where 
                is_deleted_flg <> 'Y'
            ) apd1, 
            (
        ...
        (
          with gjl_gjh as(
            SELECT 
              ...
              gjl.period_name, 
              gjl.created_by, 
              gjl.je_line_num GL_LINE_NUMBER --,to_char(gjl.effective_date,'YYYY-MM-DD HH:MI:SS') effective_date
              , 
              gjl.effective_date, 
          ...
          gjst as (
            ...
            WHERE 
              gjst.language = 'US' --      and gjst.last_update_date >= to_date('2020-01-01 12:00:00','YYYY-MM-DD HH:MI:SS')
              ), 
          -------------------------------------------------------------------------------------------------------------
          gdct as (
            SELECT 
            ...
            FROM 
              (
                ...
                where 
                  is_deleted_flg <> 'Y'
              ) gdct --     WHERE  gdct.last_update_date >= to_date('2010-01-01 12:00:00','YYYY-MM-DD HH:MI:SS')
              ), 
          -------------------------------------------------------------------------------------------------------------
          gjb_papf as (
            SELECT 
          ...
          WHERE 
            ...
            AND gjb_papf.je_batch_id = gjl_gjh.je_batch_id 
            AND fsv1.seq_version_id(+) = gjl_gjh.posting_acct_seq_version_id 
            AND fsv2.seq_version_id(+) = gjl_gjh.close_acct_seq_version_id 
      ...
      UNION ALL 
        (
          SELECT 
            ...
            0 TEMP_LINE_NUM 
          FROM 
            bec_dwh.FACT_XLA_REPORT_BALANCES_STG glbgt --,fnd_user                         fdu
            , 
            (
              select 
                * 
              from 
                bec_ods.fnd_new_messages 
              where 
                is_deleted_flg <> 'Y'
            ) fnm, 
            (
