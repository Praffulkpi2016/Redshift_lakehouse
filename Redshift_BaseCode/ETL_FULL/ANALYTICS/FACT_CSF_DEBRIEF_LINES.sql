...
drop table if exists bec_dwh.FACT_CSF_DEBRIEF_LINES;

create table bec_dwh.FACT_CSF_DEBRIEF_LINES distkey(debrief_line_id) sortkey(debrief_header_id,debrief_line_id) as 
SELECT 
  cdl.channel_code,
  cdl.debrief_line_id,
  ...
  || '-' || nvl(cdl.debrief_header_id, 0)
  || '-' || nvl(cdl.debrief_line_id, 0) as dw_load_id,
  getdate() as dw_insert_date, 
  getdate() as dw_update_date
FROM
    (select * from bec_ods.csf_debrief_lines where is_deleted_flg <> 'Y') cdl,
