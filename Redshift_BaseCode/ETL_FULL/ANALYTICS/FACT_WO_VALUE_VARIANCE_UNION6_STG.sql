...
drop table if exists bec_dwh.FACT_WO_VALUE_VARIANCE_UNION6_STG;

create table bec_dwh.FACT_WO_VALUE_VARIANCE_UNION6_STG diststyle all 
sortkey(organization_id,inventory_item_id,primary_item_id,wip_entity_id)
as
------------------------------------------------------------------------------------
select * ,
...
where	c2.standard_cost_revision_date <= NVL (d.date_closed,(SYSDATE))
group by c2.inventory_item_id,c2.organization_id--,d.wip_entity_id
),
-------------------------------------------------------------------
cte15 as (
...
on	b.assembly_item_id  = d.primary_item_id
and b.organization_id = d.organization_id
where c1.standard_cost_revision_date <= NVL (d.date_closed,(SYSDATE))
...
on b.assembly_item_id = d.primary_item_id
AND b.organization_id = d.organization_id
where e2.creation_date <= NVL (d.date_closed,(SYSDATE))
group by e2.inventory_item_id,e2.organization_id,d.wip_entity_id
),
------------------------------------------------------------------------------------
cte_16 as (
