...
drop table if exists bec_dwh.FACT_PO_SHIPMENT_STG;

CREATE TABLE bec_dwh.FACT_PO_SHIPMENT_STG diststyle all sortkey(PO_HEADER_ID,PO_LINE_ID,PO_LINE_LOCATION_ID,
RELEASE_NUM,SHIPMENT_HEADER_ID,SHIPMENT_LINE_ID,ITEM_ID) 
as ( select 
    shp.SOURCE_TYPE,
    shp.CREATED_BY,
    shp.CREATION_DATE,
...
(SELECT
		...
		(
			...
			where
			   ...
			   || '-' || nvl(rsh.vendor_id, 0)
			--   || '-' || nvl(rsl.PO_HEADER_ID, 0)
			--   || '-' || nvl(RSL.PO_LINE_ID, 0) 
			 --  || '-' || nvl(RSL.PO_LINE_LOCATION_ID, '0')	
      ...
      --  AND decode(hr_security.view_all, 'Y', 'TRUE', hr_security.show_person(he.person_id(+), he.current_applicant_flag(+), he.current_employee_flag
      --  (+), he.current_npw_flag(+), he.employee_number(+),
       --                                                                       he.applicant_number(+), he.npw_number(+))) = 'TRUE'
       -- AND decode(hr_general.get_xbg_profile, 'Y', he.business_group_id(+), hr_general.get_business_group_id) = he.business_group_id
       -- (+)
        ...
        --AND nvl(reql.org_id, - 99) = nvl(reqh.org_id, - 99)
    ...
    SELECT
        ...
        rsl.destination_context,
        rsl.destination_type_code,
        NULL destination_type,--polc.po_type_display    destination_type,
        rsl.employee_id,
        rsl.from_organization_id,
        rsl.item_description,
        rsl.item_id,
		...
		(
			...
			--   || '-' || nvl(poh.PO_HEADER_ID, 0)
			 ...
			 --  || '-' || nvl(poll.PO_LINE_LOCATION_ID, '0')	
			   || '-' || nvl(RSL.SHIPMENT_HEADER_ID, 0) 
			   || '-' || nvl(RSL.SHIPMENT_LINE_ID, '0')	  
			   || '-' || nvl(rsl.routing_header_id, '0')	as dw_load_id,  
			getdate() as dw_insert_date,
