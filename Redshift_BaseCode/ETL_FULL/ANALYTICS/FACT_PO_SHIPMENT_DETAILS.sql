...
drop table if exists bec_dwh.FACT_PO_SHIPMENT_DETAILS;

create table bec_dwh.FACT_PO_SHIPMENT_DETAILS diststyle all sortkey (PO_HEADER_ID,PO_LINE_ID)
as 	(select distinct
          POLL.ORG_ID,
          POH.PO_HEADER_ID,
          POL.PO_LINE_ID,
		  ...
		  to_char(POLL.PROMISED_DATE + DECODE(POH.TERMS_ID, 10002, 10, 10003, 15,
                                        10004, 20, 10005, 25, 10006,
                                        28, 10007, 30, 10008, 45,
                                        10009, 60, 10010, 7, 10120,
                                        120, 10100, 90, 0) ,'MON-YY') PAYMENT_MONTH,
