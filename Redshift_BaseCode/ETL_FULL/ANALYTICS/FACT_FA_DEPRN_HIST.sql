...
drop table if exists bec_dwh.FACT_FA_DEPRN_HIST;

create table bec_dwh.FACT_FA_DEPRN_HIST 
	diststyle all
	sortkey (ASSET_ID,DISTRIBUTION_ID)
as
select
	asset_id,
	book_type_code,
...
select
	...
	'' bonus_deprn_expense_acct,
	/*(select assigned_to from bec_ods.fa_distribution_history fdh 
	 where distribution_id = (select max(distribution_id) from bec_ods.fa_distribution_history
	where asset_id = dh.asset_id))  assigned_to,
	(select location_id from bec_ods.fa_distribution_history fdh 
	 where distribution_id = (select max(distribution_id) from bec_ods.fa_distribution_history
	where asset_id = dh.asset_id))  location_id, */
