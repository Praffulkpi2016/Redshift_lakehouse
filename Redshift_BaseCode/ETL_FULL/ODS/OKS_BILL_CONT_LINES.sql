...
DROP TABLE if exists bec_ods.OKS_BILL_CONT_LINES;

CREATE TABLE IF NOT EXISTS bec_ods.oks_bill_cont_lines
(
	id VARCHAR(50)   ENCODE lzo
	,cle_id VARCHAR(50)   ENCODE lzo
	,btn_id VARCHAR(50)   ENCODE lzo
	,date_billed_from TIMESTAMP WITHOUT TIME ZONE   ENCODE az64
	,date_billed_to TIMESTAMP WITHOUT TIME ZONE   ENCODE az64
	,sent_yn VARCHAR(3)   ENCODE lzo
	...
	,bill_action VARCHAR(9)   ENCODE lzo
	,date_next_invoice TIMESTAMP WITHOUT TIME ZONE   ENCODE az64
	...
	,averaged_yn VARCHAR(1)   ENCODE lzo
	,settled_yn VARCHAR(1)   ENCODE lzo
...
INSERT INTO bec_ods.oks_bill_cont_lines (
	...
	kca_seq_date
    FROM
        bec_ods_stg.oks_bill_cont_lines;
