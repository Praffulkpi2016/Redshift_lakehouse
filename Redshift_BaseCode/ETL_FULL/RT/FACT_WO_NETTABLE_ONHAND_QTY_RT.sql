...
drop table if exists bec_dwh.wo_temp;

create table bec_dwh.wo_temp as select * from 
(
with on_hand_qty as (
  select 
...
drop table if exists bec_dwh_rpt.FACT_WO_NETTABLE_ONHAND_QTY_RT;

create table bec_dwh_rpt.FACT_WO_NETTABLE_ONHAND_QTY_RT diststyle auto sortkey(WIP_ENTITY_ID)
as
select 
  wo.job_no job_no, 
  wo.WIP_ENTITY_ID, 
