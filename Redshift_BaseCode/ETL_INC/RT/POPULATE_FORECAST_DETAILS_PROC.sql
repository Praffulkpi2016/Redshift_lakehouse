CREATE OR REPLACE PROCEDURE POPULATE_FORECAST_DETAILS() 
 LANGUAGE plpgsql 
AS $$ 
DECLARE 
...
FOR i_dates in (
  select 
    ...
    organization_id, 
    part_number, 
    to_date(forecast_date, 'MM/DD/YYYY') forecast_date, 
    sum(current_forecast_quantity) forecast_qty 
  from 
    bec_install.forecast_data_stage 
  where 
    current_forecast_quantity > 0 
    and to_date(forecast_date, 'MM/DD/YYYY') >= getdate()
  group by 
	batch_id,
    forecast_designator, 
    organization_id, 
...
from 
  bec_dwh.fact_ascp_supply_demand fasd 
where 
  plan_id = 40029 
...
insert into bec_install.FACT_INSTALL_FORECAST_DETAILS 
values 
  (
	i_dates.batch_id,
