...
begin
execute 'delete from bec_dwh.fact_cst_onhand_qty_snp where snp_date =' || quote_literal(v_current_date);

execute 'INSERT INTO bec_dwh.fact_cst_onhand_qty_snp
(part_number,
    cat_seg1,
    cat_seg2,
    category,
...
(select part_number,
    ...
    dw_load_id,
	rownumber,'
    ||quote_literal(v_current_date)|| ' from bec_dwh.fact_cst_onhand_qty)';
	
-- Purge the Snapshot data which exceeds 90 days and keep only last date of every month.

execute 'delete from bec_dwh.fact_cst_onhand_qty_snp where 
 snp_date in (SELECT
  distinct snp_date
FROM
  bec_dwh.fact_cst_onhand_qty_snp
WHERE
  snp_date not IN (
    SELECT   MAX(snp_date)
    FROM     bec_dwh.fact_cst_onhand_qty_snp
