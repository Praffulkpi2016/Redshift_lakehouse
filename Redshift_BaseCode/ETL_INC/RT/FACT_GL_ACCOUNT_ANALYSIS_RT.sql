...
delete from 
 bec_dwh_rpt.FACT_GL_ACCOUNT_ANALYSIS_RT 
where 
   dw_load_id in (
    select 
      dw_load_id
...
commit;
-- Insert records
insert into bec_dwh_rpt.FACT_GL_ACCOUNT_ANALYSIS_RT (
"accounted_cr", 
"accounted_dr", 
...
"Include_Zero_Amount_Lines" 
  ...
  SELECT 
    ...
    CAST(
      "fact_gl_account_analysis"."header_description" AS TEXT
    ...
    CAST(
      "fact_gl_account_analysis"."line_description" AS TEXT
    ) AS "line_description", 
    ...
    decode(
      "fact_gl_account_analysis"."approver_name", 
      null, 'SYSADMIN', "fact_gl_account_analysis"."approver_name"
    ) as "user", 
    nvl(
      "fact_gl_account_analysis"."accounted_dr", 
      0
    )- nvl(
      "fact_gl_account_analysis"."accounted_cr", 
      0
    ) as net_amount, 
    case when nvl(
      "fact_gl_account_analysis"."accounted_dr", 
      ...
      "fact_gl_account_analysis"."accounted_cr", 
      0
    ) = 0 then 'Y' else 'N' end as "Include_Zero_Amount_Lines" 
  FROM 
    (select * from bec_dwh.fact_gl_account_analysis
    where Update_flg = 'Y') fact_gl_account_analysis
    INNER JOIN (
      SELECT 
      ...
      FROM 
        "bec_dwh"."dim_ledgers" "dim_ledgers"
    ) "t0" ON (
      (
...
Update bec_dwh.fact_gl_account_analysis
set Update_flg = 'N'
where Update_flg = 'Y';
