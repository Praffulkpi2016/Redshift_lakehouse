...
--Delete Records
DELETE FROM bec_dwh.FACT_PO_DETAILS
WHERE EXISTS (
    SELECT 1
    FROM bec_ods.po_headers_all a
    LEFT OUTER JOIN bec_ods.po_lines_all b ON a.po_header_id = b.po_header_id
    WHERE
        FACT_PO_DETAILS.dw_load_id = (
            SELECT system_id
            FROM bec_etl_ctrl.etlsourceappid
            WHERE source_system = 'EBS'
        ) || '-' || NVL(a.po_header_id, 0) || '-' || NVL(b.po_line_id, 0)
    AND (
        a.kca_seq_date > (
            SELECT executebegints - prune_days
            FROM bec_etl_ctrl.batch_dw_info
            WHERE dw_table_name = 'fact_po_details' AND batch_name = 'po'
        )
        OR b.kca_seq_date> (
            SELECT executebegints - prune_days
            FROM bec_etl_ctrl.batch_dw_info
...
commit;
-- Insert records
INSERT INTO bec_dwh.FACT_PO_DETAILS
(
   po_header_id,
	agent_id,
	type_lookup_code,
