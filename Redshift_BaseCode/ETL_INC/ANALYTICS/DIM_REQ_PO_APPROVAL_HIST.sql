...
begin;
--delete 
DELETE FROM bec_dwh.DIM_REQ_PO_APPROVAL_HIST
WHERE EXISTS
(select 1 
from 
(
...
where 1=1
        AND nvl(DIM_REQ_PO_APPROVAL_HIST.requisition_id, 0) = nvl(ods.requisition_id, 0)
        AND nvl(DIM_REQ_PO_APPROVAL_HIST.po_header_id, 0) = nvl(ods.po_header_id, 0)
        AND nvl(DIM_REQ_PO_APPROVAL_HIST.po_release_no, 0) = nvl(ods.po_release_no, 0)
        AND nvl(DIM_REQ_PO_APPROVAL_HIST.organization_id , 0)= nvl(ods.organization_id, 0)
    );
commit;
--insert
insert into bec_dwh.DIM_REQ_PO_APPROVAL_HIST
(
SELECT
    req_po.requisition_id,
    req_po.req_num,
