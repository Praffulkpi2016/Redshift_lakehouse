...
-- Delete Records
delete from 
  bec_dwh.FACT_ASL_SR_VALIDATION 
where 
  (
	nvl(item_id, 0), nvl(asl_id, 0) , nvl(document_header_id, 0)
,   nvl(document_line_id, 0), nvl(using_organization_id, 0)
  , nvl(owning_organization_id, 0)  )
    in (
    select 
      nvl(ods.item_id, 0) as item_id, 
	  nvl(ods.asl_id, 0) as asl_id,
	  nvl(ods.document_header_id, 0) as document_header_id, 
 	  nvl(ods.document_line_id, 0) as document_line_id,
	  nvl(ods.using_organization_id, 0) as using_organization_id,
	  nvl(ods.owning_organization_id, 0) as owning_organization_id 
    from 
      bec_dwh.FACT_ASL_SR_VALIDATION dw, 
      (
		SELECT distinct
			paa.asl_id,
		...
		FROM
			BEC_ODS.po_asl_attributes paa,
			BEC_ODS.po_approved_supplier_list pasl,
			BEC_ODS.po_asl_documents pad,
			BEC_ODS.po_headers_all   ph,
			BEC_ODS.po_lines_all   pl,
			BEC_ODS.mtl_system_items_b msi,
			BEC_ODS.fnd_lookup_values ml1,
			BEC_ODS.fnd_lookup_values ml2,
 			BEC_ODS.mtl_item_sub_defaults dsub, 
			BEC_ODS.fnd_lookup_values plc,
			BEC_ODS.fnd_lookup_values plc1,
			BEC_ODS.fnd_lookup_values plc2
		WHERE
    ...
    where 
      dw.dw_load_id = (
		...
		||'-'||nvl(ods.asl_id,0) 
		||'-'||nvl(ods.document_header_id,0)
		||'-'||nvl(ods.document_line_id,0) 
		||'-'||nvl(ods.using_organization_id,0)
		||'-'||nvl(ods.owning_organization_id,0)
	);
commit;
-- Insert records
insert into bec_dwh.FACT_ASL_SR_VALIDATION (
	asl_id,
	asl_status_id,
	buyer_id,
