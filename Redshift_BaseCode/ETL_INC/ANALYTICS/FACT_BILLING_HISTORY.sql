...
*/
begin;
delete from bec_dwh.FACT_BILLING_HISTORY
where EXISTS
(
select 1
FROM
...
bec_ods.hz_cust_acct_sites_all lhcasa_shipto,
bec_ods.hz_cust_site_uses_all lhcsua_shipto,
bec_ods.hz_parties lhz_shipto,
bec_ods.hz_locations lhl,
bec_ods.hz_party_sites lhps,
bec_ods.ra_customer_trx_all ct,
bec_ods.hz_party_sites hps,
bec_ods.hz_locations hl,
bec_ods.ra_batch_sources_all rbsa,
bec_ods.ra_cust_trx_types_all rctta,
bec_ods.ra_cust_trx_line_gl_dist_all rgd,
bec_ods.ra_terms_tl rt,
bec_ods.ra_customer_trx_lines_all rctla,
	(
	select
			a.cognomen as cognomen,
			b.contract_number
	from bec_ods.okc_k_headers_tl  a,
		bec_ods.okc_k_headers_all_b  b
	where
			a.id = b.id
		and
	...
	from
			(select * from bec_ods.ra_customer_trx_lines_all where 1=1) t,
			(select * from bec_ods.ra_cust_trx_line_gl_dist_all where 1=1) rgd
	where
			1 = 1
...
--			tl.customer_trx_id,
	...
	from
			(select * from bec_ods.ra_customer_trx_lines_all where 1=1) tl
	where
			1 = 1
...
--			tl.customer_trx_id,
            ...
            (select max(nvl(gl_date,'1900-01-01 12:00:00')) gl_date,
                rgds.customer_trx_line_id from (select * from bec_ods.ra_cust_trx_line_gl_dist_all where 1=1) rgds
----    where  rctla.customer_trx_line_id = rgds.customer_trx_line_id 
    group by rgds.customer_trx_line_id )rgds_gl
WHERE 1=1
	and (ct.kca_seq_date > (select (executebegints-prune_days) from bec_etl_ctrl.batch_dw_info 
where dw_table_name ='fact_billing_history' and batch_name = 'ar')
or 
rctla.kca_seq_date > (select (executebegints-prune_days) from bec_etl_ctrl.batch_dw_info 
where dw_table_name ='fact_billing_history' and batch_name = 'ar')
or 
rgd.kca_seq_date > (select (executebegints-prune_days) from bec_etl_ctrl.batch_dw_info 
where dw_table_name ='fact_billing_history' and batch_name = 'ar'))
...
commit;
insert into bec_dwh.FACT_BILLING_HISTORY
(
SELECT 
bill_to_account
...
FROM
    ...
    (select customer_trx_line_id,CUST_TRX_LINE_GL_DIST_ID,code_combination_id , kca_seq_date from bec_ods.ra_cust_trx_line_gl_dist_all where is_deleted_flg <> 'Y')   rgd,
    	(
	select
		*
	from
