...
begin;
--delete 
delete from bec_dwh.FACT_MATERIAL_TRANSACTIONS
where exists 
(select 1
from
bec_ods.mtl_material_transactions  mmt,
bec_ods.mtl_system_items_b  msi,
(select distinct transaction_id,inventory_item_id,organization_id,serial_number 
from bec_ods.mtl_unit_transactions)  mut,
...
bec_ods.mtl_item_locations  mil1,
bec_ods.mtl_item_locations  mil2
where
...
where dw_table_name ='fact_material_transactions' and batch_name = 'inv')
)
...
--INSERT
insert into  bec_dwh.FACT_MATERIAL_TRANSACTIONS 
select
	msi.segment1,
	msi.description,
