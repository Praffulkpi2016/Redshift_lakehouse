...
begin;
--delete
truncate table bec_dwh.FACT_INV_OHQ_DETAILS_TMP;

insert into bec_dwh.FACT_INV_OHQ_DETAILS_TMP
(
...
where dw_table_name ='fact_inv_ohq_details' and batch_name = 'inv')
    UNION ALL 
    SELECT 
...
where dw_table_name ='fact_inv_ohq_details' and batch_name = 'inv')
 ...
 delete from bec_dwh.FACT_INV_OHQ_DETAILS 
 where (inventory_item_id,organization_id) in 
 (select inventory_item_id,organization_id from bec_dwh.FACT_INV_OHQ_DETAILS_TMP);
 commit;
 
 --insert 
insert into bec_dwh.FACT_INV_OHQ_DETAILS 
SELECT 
  part_number, 
  description, 
...
FROM 
  (
    ...
    FROM 
	  ...
	  bec_dwh.FACT_INV_OHQ_DETAILS_TMP tmp
    WHERE 
