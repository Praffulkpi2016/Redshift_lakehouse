...
drop table if exists bec_dwh.DIM_SEGMENT1;

create table bec_dwh.DIM_SEGMENT1 diststyle all sortkey(chart_of_accounts_id,
ledger_id)
as
(
select
	...
	concat(f1.flex_value, concat('-', f1.description)) as seg1_value_desc,
...
from
	...
	select 
		...
		f2.description
	from
		(SELECT * FROM bec_ods.fnd_flex_values where is_deleted_flg <> 'Y') f1
...
(
	...
	from
		(SELECT * FROM bec_ods.fnd_id_flex_structures_vl where is_deleted_flg <> 'Y')  f1
