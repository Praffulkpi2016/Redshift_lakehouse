...
delete from  bec_dwh.FACT_XLA_REPORT_BALANCES_STG
where (
nvl(LEDGER_ID, 0)
,nvl(BUDGET_NAME,'NA')
,nvl(PERIOD_NAME,'NA')
...
(
SELECT
 nvl(ods.LEDGER_ID, 0) LEDGER_ID
,nvl(ods.BUDGET_NAME,'NA')  BUDGET_NAME
,nvl(ods.PERIOD_NAME,'NA') PERIOD_NAME
,nvl(ods.CODE_COMBINATION_ID,0) CODE_COMBINATION_ID
,nvl(ods.LEDGER_CURRENCY,'NA') LEDGER_CURRENCY
,nvl(ods.TRANSLATED_FLAG,'NA') TRANSLATED_FLAG
from bec_dwh.FACT_XLA_REPORT_BALANCES_STG dw,
(
SELECT 
...
--              ,glb.code_combination_id)   CODE_COMBINATION_DESCRIPTION
       ...
       FROM (SELECT 
                   ...
                   ,gls.start_date
                   ,gls.end_date
               FROM bec_ods.gl_ledgers                         gll
                   ,bec_ods.gl_period_statuses                 gls
           ...
           ,bec_ods.gl_balances                        glb
           ,bec_ods.gl_code_combinations               gcck
           ,bec_ods.fnd_lookup_values  xlk
           ,bec_ods.gl_budget_versions                 glv
           ,bec_ods.gl_encumbrance_types               get
...
(
	...
	where
		source_system = 'EBS')
		|| '-' || nvl(ods.LEDGER_ID, 0)
		|| '-' || nvl(ods.BUDGET_NAME,'NA') 
		|| '-' || nvl(ods.PERIOD_NAME,'NA') 
		|| '-' || nvl(ods.CODE_COMBINATION_ID,0) 
		|| '-' || nvl(ods.LEDGER_CURRENCY,'NA') 
		|| '-' || nvl(ods.TRANSLATED_FLAG,'NA') 
);

commit;
-- Insert Records

INSERT INTO bec_dwh.FACT_XLA_REPORT_BALANCES_STG
(
LEDGER_ID
,LEDGER_SHORT_NAME
,LEDGER_DESCRIPTION
