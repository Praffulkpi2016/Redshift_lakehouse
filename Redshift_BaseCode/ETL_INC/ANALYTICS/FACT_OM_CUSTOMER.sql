...
begin;
--delete 
delete from bec_dwh.FACT_OM_CUSTOMER
where exists 
(
select 1
FROM   
bec_dwh.DIM_CUSTOMER_DETAILS   BCDV
,bec_ods.HZ_PARTY_SITES 		    HPS
,bec_ods.HZ_LOCATIONS 			HL
,bec_ods.HZ_CUST_ACCOUNTS 		HCAL
,bec_ods.HZ_CUST_ACCOUNTS 		HCA
,bec_ods.HZ_PARTIES             HP
,bec_ods.HZ_CUST_SITE_USES_ALL HCSU
,bec_ods.HZ_CUSTOMER_PROFILES 		HCP
,bec_ods.HZ_CUST_PROFILE_CLASSES	HCPC    
WHERE  1=1
...
and 
(BCDV.dw_update_date > (select (executebegints-prune_days) from bec_etl_ctrl.batch_dw_info 
where dw_table_name ='fact_om_customer' and batch_name = 'om')
OR
HCSU.kca_seq_date > (select (executebegints-prune_days) from bec_etl_ctrl.batch_dw_info 
where dw_table_name ='fact_om_customer' and batch_name = 'om')
)
...
--insert
insert into bec_dwh.FACT_OM_CUSTOMER
(
SELECT  
 BCDV.PARTY_SITE_ID
