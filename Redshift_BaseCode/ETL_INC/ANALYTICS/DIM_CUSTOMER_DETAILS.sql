...
begin;
--delete 
delete from bec_dwh.DIM_CUSTOMER_DETAILS
where exists 
(
select 1
FROM
...
AND HCSU.STATUS = 'A' 
and 
(nvl(HCS.kca_seq_date) > (select (executebegints-prune_days) from bec_etl_ctrl.batch_dw_info 
where dw_table_name ='dim_customer_details' and batch_name = 'om')
OR
nvl(HCSU.kca_seq_date) > (select (executebegints-prune_days) from bec_etl_ctrl.batch_dw_info 
where dw_table_name ='dim_customer_details' and batch_name = 'om')
)
and nvl(DIM_CUSTOMER_DETAILS.CUST_ACCT_SITE_ID,0) = nvl(HCS.CUST_ACCT_SITE_ID, 0)
and nvl(DIM_CUSTOMER_DETAILS.SITE_USE_ID,0) = nvl(HCSU.SITE_USE_ID, 0)
);
commit;

insert into bec_dwh.dim_customer_details
(
SELECT
HP.PARTY_NAME,
HP.PARTY_NUMBER,
...
commit;
--soft delete 
update bec_dwh.DIM_CUSTOMER_DETAILS
...
(select * from BEC_ODS.HZ_CUST_ACCT_SITES_ALL where is_deleted_flg <> 'Y')HCS,
