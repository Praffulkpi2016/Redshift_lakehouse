...
*/
begin;
truncate table bec_dwh.fact_rtv_serial_dtls_tmp;

insert into bec_dwh.fact_rtv_serial_dtls_tmp
select 
distinct
...
where 1=1
and (
mmt.kca_seq_date > (select (executebegints-prune_days) 
from bec_etl_ctrl.batch_dw_info 
where dw_table_name ='fact_rtv_serial_dtls' and batch_name = 'po')
OR
mut.kca_seq_date > (select (executebegints-prune_days) 
from bec_etl_ctrl.batch_dw_info 
where dw_table_name ='fact_rtv_serial_dtls' and batch_name = 'po')
OR
msn.kca_seq_date > (select (executebegints-prune_days) 
from bec_etl_ctrl.batch_dw_info 
where dw_table_name ='fact_rtv_serial_dtls' and batch_name = 'po'))
...
delete from bec_dwh.fact_rtv_serial_dtls
WHERE exists (select 1 from bec_dwh.fact_rtv_serial_dtls_tmp tmp
              where 1=1
			  and tmp.inventory_item_id = fact_rtv_serial_dtls.inventory_item_id
			  and tmp.organization_id = fact_rtv_serial_dtls.organization_id
			  and tmp.serial_number = fact_rtv_serial_dtls.serial_number
             );
 commit;
-- Insert records into fact table
insert into bec_dwh.FACT_RTV_SERIAL_DTLS
	(
		SELECT 
		...
		(
			(
				...
				inner join bec_dwh.fact_rtv_serial_dtls_tmp tmp
				on tmp.inventory_item_id = msi.inventory_item_id
			  and tmp.organization_id = mmt.organization_id
			  and tmp.serial_number = mut.serial_number 
