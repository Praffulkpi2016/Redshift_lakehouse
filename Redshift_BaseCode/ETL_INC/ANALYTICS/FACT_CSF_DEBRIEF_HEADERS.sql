...
begin;
--DELETE
delete from bec_dwh.FACT_CSF_DEBRIEF_HEADERS
where exists
(
select 1
FROM
bec_ods.csf_debrief_headers cdh
WHERE   cdh.kca_seq_date > (select (executebegints-prune_days) from bec_etl_ctrl.batch_dw_info 
where dw_table_name ='fact_csf_debrief_headers' and batch_name = 'inv')
and FACT_CSF_DEBRIEF_HEADERS.debrief_header_id  = cdh.debrief_header_id
);
commit;

insert into bec_dwh.FACT_CSF_DEBRIEF_HEADERS
SELECT 
  jtb.owner_type_code, 
  jtb.owner_id,
...
FROM
	(select * from bec_ods.jtf_task_statuses_tl where is_deleted_flg <> 'Y') jts1,
	(select * from bec_ods.jtf_task_statuses_tl where is_deleted_flg <> 'Y') jts2,
	...
	(select * from bec_ods.csf_debrief_headers where is_deleted_flg <> 'Y'
	and kca_seq_date > (select (executebegints-prune_days) from bec_etl_ctrl.batch_dw_info 
where dw_table_name ='fact_csf_debrief_headers' and batch_name = 'inv')) cdh,
	(select * from bec_ods.jtf_tasks_tl where is_deleted_flg <> 'Y') jtv,
	...
	(select * from bec_ods.jtf_task_assignments where is_deleted_flg <> 'Y') jta
