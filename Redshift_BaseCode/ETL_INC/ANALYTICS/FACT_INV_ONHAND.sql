...
begin;
--delete 
truncate table bec_dwh.FACT_INV_ONHAND_TMP;

insert into bec_dwh.FACT_INV_ONHAND_TMP
( select distinct inventory_item_id,organization_id from
(
SELECT   
--miq.create_transaction_id TRANSACTION_ID,
miq.inventory_item_id,
miq.organization_id
FROM bec_ods.mtl_system_items_b  msi,
bec_ods.mtl_onhand_quantities_detail  miq,
...
bec_ods.mtl_txn_source_types  mts,
...
and (miq.kca_seq_date > (select (executebegints-prune_days) 
from bec_etl_ctrl.batch_dw_info 
where dw_table_name ='fact_inv_onhand' and batch_name = 'inv')		
or 
ctc.kca_seq_date > (select (executebegints-prune_days) 
from bec_etl_ctrl.batch_dw_info 
where dw_table_name ='fact_inv_onhand' and batch_name = 'inv')	
)
UNION ALL
...
delete from bec_dwh.FACT_INV_ONHAND
where (inventory_item_id,organization_id)
in 
(select inventory_item_id,organization_id from bec_dwh.FACT_INV_ONHAND_TMP);
commit;

--INSERT
insert into  bec_dwh.FACT_INV_ONHAND
SELECT  DISTINCT 
TRANSACTION_ID,
inventory_item_id,
organization_id,
...
and  miq.subinventory_code = mq.subinventory_code		 
        ...
        FROM (select * from bec_ods.mtl_system_items_b where is_deleted_flg<>'Y') msi,
			 ...
			 bec_dwh.FACT_INV_ONHAND_TMP tmp
       WHERE tmp.inventory_item_id = miq.inventory_item_id
...
--                    bec_ods.mtl_categories mc,
                    ...
                    (select * from bec_ods.mtl_secondary_inventories where is_deleted_flg<>'Y') sub,
								 bec_dwh.FACT_INV_ONHAND_TMP tmp
       WHERE tmp.inventory_item_id = msn.inventory_item_id
