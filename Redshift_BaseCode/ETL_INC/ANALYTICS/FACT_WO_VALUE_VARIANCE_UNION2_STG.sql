...
drop table if EXISTS bec_dwh.FACT_WO_VALUE_VARIANCE_UNION2_STG_INCR;

create table bec_dwh.FACT_WO_VALUE_VARIANCE_UNION2_STG_INCR as
with mta as (
	select
		transaction_id,
...
wdj as (
	...
	where (kca_seq_date > (select (executebegints-prune_days) from bec_etl_ctrl.batch_dw_info 
		where dw_table_name = 'fact_wo_value_variance_union2_stg'
		and batch_name = 'wip'))),
------------------------------------------------------------------------------------
we as (
...
(
	select
		...
		nvl(cast(ods.inventory_item_id as int), 0) as inventory_item_id,
		nvl(ods.wip_entity_id, 0) as wip_entity_id ,
		nvl(cast(ods.operation_seq_num as int), 0) as operation_seq_num,
		nvl(cast(ods.res_op_seq_num as int), 0)  as res_op_seq_num,
		nvl(cast(ods.resource_seq_num as int), 0) as resource_seq_num,
		nvl(ods.WIP_Supply_Type, 'NA') as WIP_Supply_Type,
		nvl(ods.ALTERNATE_BOM, 'NA') as ALTERNATE_BOM,
		nvl(ods.comments, 'NA') as comments
	from
		bec_dwh.FACT_WO_VALUE_VARIANCE_UNION2_STG dw,
		bec_dwh.FACT_WO_VALUE_VARIANCE_UNION2_STG_INCR ods
	where
		1 = 1
		and dw.dw_load_id =
		(
...
-----------------------------------------------------------------------------
-----------------------------------------------------------------------------
insert into bec_dwh.FACT_WO_VALUE_VARIANCE_UNION2_STG
select * from bec_dwh.FACT_WO_VALUE_VARIANCE_UNION2_STG_INCR;
-----------------------------------------------------------------------------
-----------------------------------------------------------------------------
end;
