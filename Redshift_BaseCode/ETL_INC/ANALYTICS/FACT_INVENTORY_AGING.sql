...
begin;
	...
	(
		...
		nvl(orig_date_received, '1900-01-01 12:00:00')
	)
	in
	(
		select
		...
		nvl(ods.subinventory_code, 'NA') as subinventory_code,
		nvl(ods.locator_id, 0) as locator_id,
		nvl(ods.onhand_quantities_id, 0) as onhand_quantities_id,
		nvl(ods.orig_date_received, '1900-01-01 12:00:00') as orig_date_received
		from
		bec_dwh.FACT_INVENTORY_AGING dw,
		(
			select
			...
			from
			bec_ods.mtl_onhand_quantities_detail   moqd,
			bec_ods.mtl_system_items_b     msib,
			bec_ods.mtl_secondary_inventories  msiv,
			bec_ods.mtl_item_categories   mic,
			bec_ods.mtl_categories_b   mc,
			bec_ods.mtl_item_locations   mil,
			bec_ods.cst_cost_groups   ccg,
			bec_ods.cst_quantity_layers  cql,
		...
		where
		source_system = 'EBS')|| '-' || nvl(ods.organization_id, 0)|| '-' || nvl(ods.inventory_item_id, 0)
		|| '-' || nvl(ods.subinventory_code, 'NA')|| '-' || nvl(ods.locator_id, 0)|| '-' || nvl(ods.onhand_quantities_id, 0)|| '-' || nvl(ods.orig_date_received, '1900-01-01 12:00:00')
	);
