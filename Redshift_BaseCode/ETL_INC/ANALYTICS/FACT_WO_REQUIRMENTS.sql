...
--delete records
delete from bec_dwh.FACT_WO_REQUIRMENTS 
where 
...
(       SELECT    
	  ...
	  from bec_etl_ctrl.batch_dw_info where dw_table_name = 'fact_wo_requirments' and batch_name = 'wip')) we,
           (SELECT * FROM bec_ods.wip_discrete_jobs WHERE IS_DELETED_FLG <> 'Y'
		   AND kca_seq_date > (select (executebegints-prune_days) 
	  from bec_etl_ctrl.batch_dw_info where dw_table_name = 'fact_wo_requirments' and batch_name = 'wip')) wdj,
           (SELECT * FROM bec_ods.wip_operations WHERE IS_DELETED_FLG <> 'Y'
		   AND kca_seq_date > (select (executebegints-prune_days) 
	  from bec_etl_ctrl.batch_dw_info where dw_table_name = 'fact_wo_requirments' and batch_name = 'wip')) wo,
           (SELECT * FROM bec_ods.wip_requirement_operations WHERE IS_DELETED_FLG <> 'Y'
		   AND kca_seq_date > (select (executebegints-prune_days) 
	  from bec_etl_ctrl.batch_dw_info where dw_table_name = 'fact_wo_requirments' and batch_name = 'wip')) wro,
...
commit;
-- Insert records
insert into bec_dwh.FACT_WO_REQUIRMENTS 
(
wip_entity_id
,organization_id
,inventory_item_id
