...
delete from bec_ods.ce_bank_accounts
where BANK_ACCOUNT_ID in (
select stg.BANK_ACCOUNT_ID from bec_ods.ce_bank_accounts ods, bec_ods_stg.ce_bank_accounts stg
where ods.BANK_ACCOUNT_ID = stg.BANK_ACCOUNT_ID  and stg.kca_operation IN ('INSERT','UPDATE')  );
...
,cast(NULLIF(KCA_SEQ_ID,'') as numeric(36,0)) as KCA_SEQ_ID,
		kca_seq_date
FROM bec_ods_stg.ce_bank_accounts
where kca_operation IN ('INSERT','UPDATE') and (BANK_ACCOUNT_ID,kca_seq_id) in (select BANK_ACCOUNT_ID,max(kca_seq_id) from bec_ods_stg.ce_bank_accounts 
...
-- Soft delete
update bec_ods.ce_bank_accounts set IS_DELETED_FLG = 'N';
commit;
update bec_ods.ce_bank_accounts set IS_DELETED_FLG = 'Y'
where (BANK_ACCOUNT_ID )  in
(
select BANK_ACCOUNT_ID  from bec_raw_dl_ext.ce_bank_accounts
where (BANK_ACCOUNT_ID ,KCA_SEQ_ID)
in 
(
select BANK_ACCOUNT_ID ,max(KCA_SEQ_ID) as KCA_SEQ_ID 
...
update bec_etl_ctrl.batch_ods_info set last_refresh_date = getdate() where ods_table_name='ce_bank_accounts';
commit;
