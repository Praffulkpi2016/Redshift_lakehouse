...
delete from bec_ods.ap_invoice_distributions_all
where invoice_distribution_id in (
select stg.invoice_distribution_id 
from bec_ods.ap_invoice_distributions_all ods, bec_ods_stg.ap_invoice_distributions_all stg
where ods.invoice_distribution_id = stg.invoice_distribution_id
...
where kca_operation IN ('INSERT','UPDATE') 
	...
	(select invoice_distribution_id,max(kca_seq_id) from bec_ods_stg.ap_invoice_distributions_all 
     where kca_operation IN ('INSERT','UPDATE')
...
-- Soft delete
update bec_ods.ap_invoice_distributions_all set IS_DELETED_FLG = 'N';
commit;
update bec_ods.ap_invoice_distributions_all set IS_DELETED_FLG = 'Y'
where (invoice_distribution_id)  in
(
select invoice_distribution_id from bec_raw_dl_ext.ap_invoice_distributions_all
where (invoice_distribution_id,KCA_SEQ_ID)
in 
(
select invoice_distribution_id,max(KCA_SEQ_ID) as KCA_SEQ_ID 
...
update bec_etl_ctrl.batch_ods_info set last_refresh_date = getdate() where ods_table_name='ap_invoice_distributions_all';
commit;