...
delete from bec_ods.msc_system_items
where (SR_INSTANCE_ID,PLAN_ID,INVENTORY_ITEM_ID,ORGANIZATION_ID) in (
select stg.SR_INSTANCE_ID,stg.PLAN_ID,stg.INVENTORY_ITEM_ID,stg.ORGANIZATION_ID from bec_ods.msc_system_items ods, bec_ods_stg.msc_system_items stg where 
ods.SR_INSTANCE_ID = stg.SR_INSTANCE_ID and 
ods.PLAN_ID = stg.PLAN_ID and
ods.INVENTORY_ITEM_ID = stg.INVENTORY_ITEM_ID and
ods.ORGANIZATION_ID = stg.ORGANIZATION_ID and
...
(
	...
	kca_seq_date
	from bec_ods_stg.msc_system_items
	where kca_operation IN ('INSERT','UPDATE') 
	and (SR_INSTANCE_ID,PLAN_ID,INVENTORY_ITEM_ID,ORGANIZATION_ID,kca_seq_id) in 
	(select SR_INSTANCE_ID,PLAN_ID,INVENTORY_ITEM_ID,ORGANIZATION_ID,max(kca_seq_id) from bec_ods_stg.msc_system_items 
     where kca_operation IN ('INSERT','UPDATE')
...
-- Soft delete
update bec_ods.msc_system_items set IS_DELETED_FLG = 'N';
commit;
update bec_ods.msc_system_items set IS_DELETED_FLG = 'Y'
where (SR_INSTANCE_ID,PLAN_ID,INVENTORY_ITEM_ID,ORGANIZATION_ID)  in
(
select SR_INSTANCE_ID,PLAN_ID,INVENTORY_ITEM_ID,ORGANIZATION_ID from bec_raw_dl_ext.msc_system_items
where (SR_INSTANCE_ID,PLAN_ID,INVENTORY_ITEM_ID,ORGANIZATION_ID,KCA_SEQ_ID)
in 
(
select SR_INSTANCE_ID,PLAN_ID,INVENTORY_ITEM_ID,ORGANIZATION_ID,max(KCA_SEQ_ID) as KCA_SEQ_ID 
...
where ods_table_name = 'msc_system_items';
