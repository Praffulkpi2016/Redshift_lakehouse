...
delete from bec_ods.mtl_system_items_b
where (INVENTORY_ITEM_ID,ORGANIZATION_ID) in (
select stg.INVENTORY_ITEM_ID,stg.ORGANIZATION_ID from bec_ods.mtl_system_items_b ods, bec_ods_stg.mtl_system_items_b stg
where ods.INVENTORY_ITEM_ID = stg.INVENTORY_ITEM_ID and ods.ORGANIZATION_ID = stg.ORGANIZATION_ID and stg.kca_operation in ('INSERT','UPDATE'));
...
where kca_operation IN ('INSERT','UPDATE') and (INVENTORY_ITEM_ID,ORGANIZATION_ID,kca_seq_id) in (select INVENTORY_ITEM_ID,ORGANIZATION_ID,max(kca_seq_id) 
from bec_ods_stg.mtl_system_items_b 
...
-- Soft delete
update bec_ods.mtl_system_items_b set IS_DELETED_FLG = 'N';
commit;
update bec_ods.mtl_system_items_b set IS_DELETED_FLG = 'Y'
where (INVENTORY_ITEM_ID,ORGANIZATION_ID)  in
(
select INVENTORY_ITEM_ID,ORGANIZATION_ID from bec_raw_dl_ext.mtl_system_items_b
where (INVENTORY_ITEM_ID,ORGANIZATION_ID,KCA_SEQ_ID)
in 
(
select INVENTORY_ITEM_ID,ORGANIZATION_ID,max(KCA_SEQ_ID) as KCA_SEQ_ID 
...
update bec_etl_ctrl.batch_ods_info set last_refresh_date = getdate() where ods_table_name='mtl_system_items_b';
commit;