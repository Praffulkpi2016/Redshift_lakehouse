...
delete from bec_ods.po_headers_all
where PO_HEADER_ID in (
select stg.PO_HEADER_ID from bec_ods.po_headers_all ods, bec_ods_stg.po_headers_all stg
where ods.PO_HEADER_ID = stg.PO_HEADER_ID and stg.kca_operation in ('INSERT','UPDATE'));
...
where kca_operation in ('INSERT','UPDATE') and (PO_HEADER_ID,kca_seq_id) in (select PO_HEADER_ID,max(kca_seq_id) from bec_ods_stg.po_headers_all 
...
-- Soft delete
update bec_ods.po_headers_all set IS_DELETED_FLG = 'N';
commit;
update bec_ods.po_headers_all set IS_DELETED_FLG = 'Y'
where (PO_HEADER_ID)  in
(
select PO_HEADER_ID from bec_raw_dl_ext.po_headers_all
where (PO_HEADER_ID,KCA_SEQ_ID)
in 
(
select PO_HEADER_ID,max(KCA_SEQ_ID) as KCA_SEQ_ID 
...
where ods_table_name='po_headers_all';
