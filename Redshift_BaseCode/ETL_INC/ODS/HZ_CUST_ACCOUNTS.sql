...
delete from bec_ods.HZ_CUST_ACCOUNTS
where cust_account_id in (
select stg.cust_account_id from bec_ods.HZ_CUST_ACCOUNTS ods, bec_ods_stg.HZ_CUST_ACCOUNTS stg
where ods.cust_account_id = stg.cust_account_id and stg.kca_operation IN ('INSERT','UPDATE')
);
...
(
	...
	kca_seq_date
	from bec_ods_stg.HZ_CUST_ACCOUNTS
	where kca_operation IN ('INSERT','UPDATE') 
	and (cust_account_id,kca_seq_id) in 
	(select cust_account_id,max(kca_seq_id) from bec_ods_stg.HZ_CUST_ACCOUNTS 
     where kca_operation IN ('INSERT','UPDATE')
...
-- Soft delete
update bec_ods.HZ_CUST_ACCOUNTS set IS_DELETED_FLG = 'N';
commit;
update bec_ods.HZ_CUST_ACCOUNTS set IS_DELETED_FLG = 'Y'
where (cust_account_id)  in
(
select cust_account_id from bec_raw_dl_ext.HZ_CUST_ACCOUNTS
where (cust_account_id,KCA_SEQ_ID)
in 
(
select cust_account_id,max(KCA_SEQ_ID) as KCA_SEQ_ID 
...
where ods_table_name = 'hz_cust_accounts';
