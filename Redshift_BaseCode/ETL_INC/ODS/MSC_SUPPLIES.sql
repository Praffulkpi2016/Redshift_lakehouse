...
delete from bec_ods.MSC_SUPPLIES
where (nvl(PLAN_ID,0),nvl(SR_INSTANCE_ID,0),nvl(TRANSACTION_ID,0))  in (
select nvl(stg.PLAN_ID,0) as PLAN_ID,nvl(stg.SR_INSTANCE_ID,0) as SR_INSTANCE_ID,nvl(stg.TRANSACTION_ID,0) as TRANSACTION_ID from bec_ods.MSC_SUPPLIES ods, bec_ods_stg.MSC_SUPPLIES stg
where nvl(ods.PLAN_ID,0) = nvl(stg.PLAN_ID,0)
and nvl(ods.SR_INSTANCE_ID,0) = nvl(stg.SR_INSTANCE_ID,0) and 
...
(
	...
	weight_capacity_overload,
       ...
       'N' AS IS_DELETED_FLG,
		...
		kca_seq_date
	from bec_ods_stg.MSC_SUPPLIES
	where kca_operation IN ('INSERT','UPDATE') 
	and (nvl(PLAN_ID,0),nvl(SR_INSTANCE_ID, 0 ),nvl(TRANSACTION_ID, 0 ),kca_seq_id) in 
	(select nvl(PLAN_ID,0) as PLAN_ID,nvl(SR_INSTANCE_ID,0) as SR_INSTANCE_ID, nvl(TRANSACTION_ID, 0 ) as TRANSACTION_ID,max(kca_seq_id) from bec_ods_stg.MSC_SUPPLIES 
     where kca_operation IN ('INSERT','UPDATE')
...
-- Soft delete
update bec_ods.MSC_SUPPLIES set IS_DELETED_FLG = 'N';
commit;
update bec_ods.MSC_SUPPLIES set IS_DELETED_FLG = 'Y'
where (nvl(PLAN_ID,0),nvl(SR_INSTANCE_ID,0),nvl(TRANSACTION_ID,0))  in
(
select nvl(PLAN_ID,0),nvl(SR_INSTANCE_ID,0),nvl(TRANSACTION_ID,0) from bec_raw_dl_ext.MSC_SUPPLIES
where (nvl(PLAN_ID,0),nvl(SR_INSTANCE_ID,0),nvl(TRANSACTION_ID,0),KCA_SEQ_ID)
in 
(
select nvl(PLAN_ID,0),nvl(SR_INSTANCE_ID,0),nvl(TRANSACTION_ID,0),max(KCA_SEQ_ID) as KCA_SEQ_ID 
...
where ods_table_name = 'msc_supplies';
