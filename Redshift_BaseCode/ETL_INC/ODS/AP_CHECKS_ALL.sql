...
delete from bec_ods.ap_checks_all
where check_id in (
select stg.check_id from bec_ods.ap_checks_all ods, bec_ods_stg.ap_checks_all stg
where ods.check_id = stg.check_id and stg.kca_operation IN ('INSERT','UPDATE')
);
...
(
	...
	and (check_id,kca_seq_id) in 
	(select check_id,max(kca_seq_id) from bec_ods_stg.ap_checks_all 
     where kca_operation IN ('INSERT','UPDATE')
     group by check_id)	
);
...
-- Soft delete
update bec_ods.ap_checks_all set IS_DELETED_FLG = 'N';
update bec_ods.ap_checks_all set IS_DELETED_FLG = 'Y'
where (check_id)  in
(
select check_id from bec_raw_dl_ext.ap_checks_all
where (check_id,KCA_SEQ_ID)
in 
(
select check_id,max(KCA_SEQ_ID) as KCA_SEQ_ID 
