...
truncate
	table bec_ods_stg.pa_projects_all;
...
,KCA_SEQ_DATE
	from
		bec_raw_dl_ext.pa_projects_all
where kca_operation != 'DELETE'  and nvl(kca_seq_id,'') != '' and (PROJECT_ID,kca_seq_id) in (select PROJECT_ID,max(kca_seq_id) from bec_raw_dl_ext.pa_projects_all 
where kca_operation != 'DELETE'  and nvl(kca_seq_id,'') != ''
group by PROJECT_ID)
and 
(KCA_SEQ_DATE > (
select (executebegints-prune_days) from bec_etl_ctrl.batch_ods_info where ods_table_name = 'pa_projects_all')
            )
);		
