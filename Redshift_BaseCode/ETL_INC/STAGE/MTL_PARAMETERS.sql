...
*/
BEGIN;
TRUNCATE TABLE bec_ods_stg.MTL_PARAMETERS;

insert into bec_ods_stg.MTL_PARAMETERS
(
ORGANIZATION_ID
,LAST_UPDATE_DATE
,LAST_UPDATED_BY
...
where kca_operation != 'DELETE'  and nvl(kca_seq_id,'')!= '' and (ORGANIZATION_ID,kca_seq_id) in (select ORGANIZATION_ID,max(kca_seq_id) 
from bec_raw_dl_ext.mtl_parameters 
where kca_operation != 'DELETE'  and nvl(kca_seq_id,'')!= ''
group by ORGANIZATION_ID)
and 
( kca_seq_date > (
select (executebegints-prune_days) from bec_etl_ctrl.batch_ods_info where ods_table_name = 'mtl_parameters')
