...
TRUNCATE TABLE bec_ods_stg.AP_INVOICE_LINES_ALL;

insert into bec_ods_stg.AP_INVOICE_LINES_ALL
(
invoice_id
,line_number
...
,kca_seq_date
 ...
 where kca_operation != 'DELETE' and nvl(kca_seq_id,'')!= '' 
	...
	(select invoice_id,line_number,max(KCA_SEQ_ID) from bec_raw_dl_ext.ap_invoice_lines_all 
     where kca_operation != 'DELETE' and nvl(kca_seq_id,'')!= ''
     group by invoice_id,line_number)
     and  kca_seq_date > (select (executebegints-prune_days) from bec_etl_ctrl.batch_ods_info where ods_table_name ='ap_invoice_lines_all')
);
END;