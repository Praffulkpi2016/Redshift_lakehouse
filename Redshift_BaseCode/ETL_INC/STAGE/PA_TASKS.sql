...
truncate
	table bec_ods_stg.pa_tasks;

insert
	into
	bec_ods_stg.pa_tasks
(TASK_ID,
PROJECT_ID,
TASK_NUMBER,
CREATION_DATE,
...
,KCA_SEQ_DATE
	from
		bec_raw_dl_ext.pa_tasks
where kca_operation != 'DELETE'  and nvl(kca_seq_id,'') != '' and (TASK_ID,kca_seq_id) in (select TASK_ID,max(kca_seq_id) from bec_raw_dl_ext.pa_tasks 
where kca_operation != 'DELETE'  and nvl(kca_seq_id,'') != ''
group by TASK_ID)
and 
(KCA_SEQ_DATE > (
select (executebegints-prune_days) from bec_etl_ctrl.batch_ods_info where ods_table_name = 'pa_tasks')
            )
);
end;
