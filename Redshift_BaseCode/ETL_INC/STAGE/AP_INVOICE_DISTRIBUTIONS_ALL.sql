...
TRUNCATE TABLE bec_ods_stg.AP_INVOICE_DISTRIBUTIONS_ALL;

insert into bec_ods_stg.AP_INVOICE_DISTRIBUTIONS_ALL
(
accounting_date
,accounting_event_id
,accrual_posted_flag
...
,kca_seq_date
 ...
 where kca_operation != 'DELETE' and nvl(kca_seq_id,'')!= '' 
	...
	(select invoice_distribution_id,max(KCA_SEQ_ID) from bec_raw_dl_ext.AP_INVOICE_DISTRIBUTIONS_ALL 
     where kca_operation != 'DELETE' and nvl(kca_seq_id,'')!= ''
     group by invoice_distribution_id)
     and kca_seq_date > (select (executebegints-prune_days) from bec_etl_ctrl.batch_ods_info where ods_table_name ='ap_invoice_distributions_all')
);
END;