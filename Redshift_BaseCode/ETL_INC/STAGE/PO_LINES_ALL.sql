...
truncate
	table bec_ods_stg.po_lines_all;

insert
	into
	bec_ods_stg.po_lines_all
(PO_LINE_ID,
LAST_UPDATE_DATE,
LAST_UPDATED_BY,
PO_HEADER_ID,
...
,KCA_SEQ_DATE
	from
		bec_raw_dl_ext.po_lines_all
where kca_operation != 'DELETE'  and nvl(kca_seq_id,'') != '' and (PO_LINE_ID,kca_seq_id) in (select PO_LINE_ID,max(kca_seq_id) from bec_raw_dl_ext.po_lines_all 
where kca_operation != 'DELETE'  and nvl(kca_seq_id,'') != ''
group by PO_LINE_ID)
and 
(KCA_SEQ_DATE > (
select (executebegints-prune_days) from bec_etl_ctrl.batch_ods_info where ods_table_name = 'po_lines_all')
