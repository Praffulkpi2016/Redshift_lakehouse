...
TRUNCATE TABLE bec_ods_stg.AP_CHECK_FORMATS;

insert into	bec_ods_stg.AP_CHECK_FORMATS
(CHECK_FORMAT_ID
,LAST_UPDATE_DATE
,LAST_UPDATED_BY
,NAME
...
,kca_seq_date
from
	bec_raw_dl_ext.AP_CHECK_FORMATS 
	where kca_operation != 'DELETE' and nvl(kca_seq_id,'')!= '' 
	and (check_format_id,kca_seq_id) in 
	(select check_format_id,max(kca_seq_id) from bec_raw_dl_ext.AP_CHECK_FORMATS 
     where kca_operation != 'DELETE' and nvl(kca_seq_id,'')!= ''
     group by check_format_id)
     and kca_seq_date > (select (executebegints-prune_days) from bec_etl_ctrl.batch_ods_info where ods_table_name ='ap_check_formats')
	 );
END;