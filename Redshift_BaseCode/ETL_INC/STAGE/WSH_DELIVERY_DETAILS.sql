...
*/
BEGIN;
TRUNCATE TABLE bec_ods_stg.WSH_DELIVERY_DETAILS;


insert into bec_ods_stg.WSH_DELIVERY_DETAILS
(
	delivery_detail_id,
	source_code,
	source_header_id,
...
,KCA_SEQ_DATE
 ...
 where kca_operation != 'DELETE'  and nvl(kca_seq_id,'') != '' 
	...
	(select DELIVERY_DETAIL_ID,max(KCA_SEQ_ID) from bec_raw_dl_ext.WSH_DELIVERY_DETAILS 
     where kca_operation != 'DELETE'  and nvl(kca_seq_id,'') != ''
     group by DELIVERY_DETAIL_ID)
     and  (KCA_SEQ_DATE > (select (executebegints-prune_days) from bec_etl_ctrl.batch_ods_info where ods_table_name ='wsh_delivery_details')
)
);
END;